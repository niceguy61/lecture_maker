# Troubleshooting: {service_name} 문제 해결 가이드

## 📋 문서 정보
- **Day**: {day_number}
- **주요 서비스**: {primary_services}
- **작성일**: {date}
...
**대시보드 이름**: {service_name}-monitoring-dashboard

**핵심 메트릭 정의** (Day {day_number} 서비스 기준):
...
**알람 구성 예시**:
```yaml
알람명: {service_name}-{metric_name}-alert
...
### 공식 트러블슈팅 가이드
- [{service_name} Troubleshooting]({troubleshooting_guide_link})
...
### 서비스별 베스트 프랙티스
- [{service_name} Best Practices]({best_practices_link})
