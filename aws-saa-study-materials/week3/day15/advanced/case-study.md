# Robinhood - 금융 거래 처리를 위한 메시지 큐 아키텍처

> **Day 15: SQS (Simple Queue Service)**  
> **주요 AWS 서비스**: SQS, Standard Queue, FIFO Queue, Dead Letter Queue
...
- **주요 AWS 서비스**: SQS, Standard Queue, FIFO Queue, Dead Letter Queue
- **사례 출처**: https://aws.amazon.com/architecture/customers/robinhood <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
금융 거래 처리를 위한 메시지 큐 아키텍처를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > SQS
2. **주요 설정**:
   - Region: day15-resource
...
**연계 방식**: Event-Driven Integration
...
2. **SQS** → **Standard Queue** (Day 15의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **Standard Queue** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > SQS > Create Resource

**기본 설정**:
- **Name/ID**: `day15-resource`
...
**Console 경로**: Services > Compute > SQS
...
```yaml
# day15-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Robinhood - 금융 거래 처리를 위한 메시지 큐 아키텍처 - SQS 구성'

Resources:
  Day15Resource:
    Type: AWS::SQS::Resource
    Properties:
      Name: day15-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_sqs" "day15-resource" {
  name = "day15-resource"
  type = "standard"
  
  tags = {
    Project     = "day15-project"
...
**Console 경로**: CloudWatch > Metrics > SQS
...
**통합 시나리오 설명**:
- Day 15의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 15 (SQS) → Day 16 (SNS)
- Day 16 (SNS) → Day 18 (Lambda)
...
### AWS 공식 문서
- [SQS 사용 설명서](https://docs.aws.amazon.com/sqs/latest/userguide/)
- [SQS API 레퍼런스](https://docs.aws.amazon.com/sqs/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 금융 거래 처리를 위한 메시지 큐 아키텍처](https://aws.amazon.com/architecture/)
- [SQS 베스트 프랙티스](https://docs.aws.amazon.com/sqs/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [SQS 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Robinhood 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/robinhood)
- [AWS re:Invent 발표: Robinhood - 금융 거래 처리를 위한 메시지 큐 아키텍처](https://reinvent.awsevents.com/)
...
**기본 정보**:
- `15`: 일차 번호 (1-28)
- `SQS (Simple Queue Service)`: 일별 학습 주제 제목
- `Robinhood`: 기업명 (실제 또는 가상)
- `금융 거래 처리를 위한 메시지 큐 아키텍처`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `SQS, Standard Queue, FIFO Queue, Dead Letter Queue`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `SQS`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/sqs/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/sqs/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/robinhood`: 기업 블로그 URL
