# Troubleshooting: API Gateway 문제 해결 가이드
...
**대시보드 이름**: API Gateway-monitoring-dashboard

**핵심 메트릭 정의** (Day 19 서비스 기준):
...
**알람 구성 예시**:
```yaml
알람명: API Gateway-{metric_name}-alert
...
### 공식 트러블슈팅 가이드
- [API Gateway Troubleshooting]({troubleshooting_guide_link})
...
### 서비스별 베스트 프랙티스
- [API Gateway Best Practices]({best_practices_link})
