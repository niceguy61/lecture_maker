...
- **주요 AWS 서비스**: API Gateway, REST API, WebSocket API, API Keys
- **사례 출처**: https://aws.amazon.com/architecture/customers/stripe <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
결제 API 플랫폼의 게이트웨이 아키텍처를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > API Gateway
2. **주요 설정**:
   - Region: day19-resource
...
2. **API Gateway** → **REST API** (Day 19의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **REST API** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > API Gateway > Create Resource

**기본 설정**:
- **Name/ID**: `day19-resource`
...
**Console 경로**: Services > Compute > API Gateway

**연결 설정**:
1. API Gateway에서 생성한 리소스 선택
...
```yaml
# day19-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Stripe - 결제 API 플랫폼의 게이트웨이 아키텍처 - API Gateway 구성'
...
```hcl
# main.tf
resource "aws_api_gateway" "day19-resource" {
  name = "day19-resource"
  type = "standard"
  
  tags = {
    Project     = "day19-project"
...
**Console 경로**: CloudWatch > Metrics > APIGateway
...
**통합 시나리오 설명**:
- Day 19의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 19 (API Gateway) → Day 18 (Lambda)
- Day 18 (Lambda) → Day 2 (IAM Users)
...
### AWS 공식 문서
- [API Gateway 사용 설명서](https://docs.aws.amazon.com/api-gateway/latest/userguide/)
- [API Gateway API 레퍼런스](https://docs.aws.amazon.com/api-gateway/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 결제 API 플랫폼의 게이트웨이 아키텍처](https://aws.amazon.com/architecture/)
- [API Gateway 베스트 프랙티스](https://docs.aws.amazon.com/api-gateway/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [API Gateway 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Stripe 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/stripe)
- [AWS re:Invent 발표: Stripe - 결제 API 플랫폼의 게이트웨이 아키텍처](https://reinvent.awsevents.com/)
...
### 1. API Gateway의 실제 활용 방법
- API Gateway의 핵심 기능 이해
- 실제 프로덕션 환경 구성 방법
- 모범 사례 및 안티 패턴
...
**기본 정보**:
- `19`: 일차 번호 (1-28)
- `API Gateway`: 일별 학습 주제 제목
- `Stripe`: 기업명 (실제 또는 가상)
- `결제 API 플랫폼의 게이트웨이 아키텍처`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `API Gateway, REST API, WebSocket API, API Keys`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `API Gateway`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/api-gateway/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/api-gateway/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/stripe`: 기업 블로그 URL
