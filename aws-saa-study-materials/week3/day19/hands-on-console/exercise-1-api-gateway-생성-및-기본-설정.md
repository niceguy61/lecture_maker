...
## 🎯 실습 목표
AWS Console을 통해 API Gateway를 생성하고 기본 설정을 구성합니다.
...
**Console 경로**: `Services > Application Integration > API Gateway`

1. AWS Console 상단의 **Services** 메뉴 클릭
2. **Application Integration** 카테고리에서 **API Gateway** 선택
3. 화면 상단의 **"Create api"** 또는 **"Launch api"** 버튼 클릭
...
**상태 확인 방법**:
- Console 경로: `API Gateway > Resources` 또는 `API Gateway > Dashboard`
...
- [ ] API Gateway 리소스가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
...
#### 1. API Gateway 리소스 삭제

**Console 경로**: `API Gateway > Resources`

1. 생성한 리소스 선택: `day19-api-gateway`
2. **Actions** > **Delete** 클릭
