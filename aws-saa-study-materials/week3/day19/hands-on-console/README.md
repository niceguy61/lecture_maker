# Hands-On Console: API Gateway 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 19에서 학습한 **API Gateway**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- API Gateway의 핵심 기능을 Console에서 직접 구성
...
- **예상 비용**: $$0.50 - $3.00 (일반적인 실습 비용)
  ...
  - Free Tier 초과: API Gateway
- **소요 시간**: 약 60-90분분
- **권장 리전**: ap-northeast-2 (서울)
- **필요한 권한**: API GatewayFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- API Gateway를 중심으로 한 실습 환경
- 주요 구성 요소: API Gateway, REST API, WebSocket API
...
1. **API Gateway 리소스 생성**: AWS Console을 통해 API Gateway를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: API Gateway 생성 및 기본 설정
**목표**: AWS Console을 통해 API Gateway를 생성하고 기본 설정을 구성합니다.
...
1. **API Gateway 생성**
   - Console 경로: Services > Networking > API Gateway
   - 화면 상단의 "Create api" 버튼 클릭
   
2. **기본 설정**
   - **Name**: `day19-api-gateway`
     - 명명 규칙: day 번호 + 리소스 유형
     - 예시: `day8-my-bucket`
...
### Exercise 2: REST API 통합
**목표**: API Gateway와 다른 서비스를 통합합니다.
...
1. **REST API 생성**
   - Console 경로: Services > Management > REST API
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day19-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **API Gateway와 연결**
   - "API Gateway integration" 또는 "Attach API Gateway" 섹션으로 이동
   - 이전에 생성한 `day19-api-gateway` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day19-api-gateway-role`
       - 정책: API GatewayFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: API Gateway > api-gateway > Integrations
     - REST API 연결 상태: Connected

**검증**:
- [ ] REST API가 정상적으로 생성되었는가?
- [ ] API Gateway와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: API Gateway > api-gateway
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day19-API Gateway-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day19-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day19-{metric_name}-alert`
     - Alarm description: API Gateway 성능 알람
...
2. **기능 테스트**
   - Console 경로: {service} > api-gateway
   - 기본 기능 테스트:
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day19-API Gateway-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/api/gateway
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: API Gateway 생성 완료
- [ ] Exercise 2: REST API 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day19-API Gateway-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. REST API 삭제
- Console 경로: REST API > Resources
- 선택: `day19-monitoring`
...
#### 3. API Gateway 삭제
- Console 경로: API Gateway > Resources
- 선택: `day19-api-gateway`
- Actions > Delete
- 확인 옵션:
...
- 검색: `day19` 또는 `API Gateway`
- 선택: 관련 로그 그룹
- Actions > Delete log group(s)
- 확인: "Delete" 버튼 클릭

### 정리 확인 체크리스트
- [ ] 모든 API Gateway 리소스 삭제 완료
- [ ] 모든 REST API 리소스 삭제 완료
...
### 핵심 개념
1. **API Gateway 아키텍처**: API Gateway의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 19 Theory](../theory.md) - API Gateway 기본 개념
- [Day 18 Theory](../../day18/theory.md) - 사전 학습 내용
- [Day 20 Theory](../../day20/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Stripe의 API Gateway 활용 사례
- [Best Practices](../best-practices.md) - API Gateway 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [API Gateway User Guide](https://docs.aws.amazon.com/api-gateway/latest/userguide/)
- [API Gateway API Reference](https://docs.aws.amazon.com/api-gateway/latest/APIReference/)
- [AWS Console User Guide](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/)
- [Getting Started with API Gateway](https://docs.aws.amazon.com/api-gateway/latest/gsg/)
