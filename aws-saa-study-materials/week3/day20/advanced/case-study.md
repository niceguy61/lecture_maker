...
- **주요 AWS 서비스**: Step Functions, EventBridge, State Machines, Event Bus
- **사례 출처**: https://aws.amazon.com/architecture/customers/instacart <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
주문 처리 워크플로우 오케스트레이션를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > Step Functions
2. **주요 설정**:
   - Region: day20-resource
...
sequenceDiagram
    ...
    participant Service as Step Functions
...
**플로우 설명**:
1. **사용자 요청** → Step Functions
   - 사용자 요청을 받아 처리 시작
   
2. **Step Functions** → **EventBridge** (Day 20의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **EventBridge** → **Data Storage**
   - 데이터 저장 및 영속화
...
#### 1단계: Step Functions 생성

**Console 경로**: Services > Compute > Step Functions > Create Resource

**기본 설정**:
- **Name/ID**: `day20-resource`
...
**Console 경로**: Services > Compute > Step Functions

**연결 설정**:
1. Step Functions에서 생성한 리소스 선택
...
```yaml
# day20-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Instacart - 주문 처리 워크플로우 오케스트레이션 - Step Functions 구성'

Resources:
  Day20Resource:
    Type: AWS::StepFunctions::Resource
    Properties:
      Name: day20-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_step_functions" "day20-resource" {
  name = "day20-resource"
  type = "standard"
  
  tags = {
    Project     = "day20-project"
...
**Console 경로**: CloudWatch > Metrics > StepFunctions
...
```mermaid
graph LR
    D20[Day 20:<br/>Step Functions 및 EventBridge]
...
**통합 시나리오 설명**:
- Day 20의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 20 (Step Functions) → Day 18 (Lambda)
- Day 18 (Lambda) → Day 15 (SQS)
...
### AWS 공식 문서
- [Step Functions 사용 설명서](https://docs.aws.amazon.com/step-functions/latest/userguide/)
- [Step Functions API 레퍼런스](https://docs.aws.amazon.com/step-functions/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 주문 처리 워크플로우 오케스트레이션](https://aws.amazon.com/architecture/)
- [Step Functions 베스트 프랙티스](https://docs.aws.amazon.com/step-functions/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [Step Functions 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Instacart 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/instacart)
- [AWS re:Invent 발표: Instacart - 주문 처리 워크플로우 오케스트레이션](https://reinvent.awsevents.com/)
...
### 1. Step Functions의 실제 활용 방법
- Step Functions의 핵심 기능 이해
- 실제 프로덕션 환경 구성 방법
- 모범 사례 및 안티 패턴
...
**기본 정보**:
- `20`: 일차 번호 (1-28)
- `Step Functions 및 EventBridge`: 일별 학습 주제 제목
- `Instacart`: 기업명 (실제 또는 가상)
- `주문 처리 워크플로우 오케스트레이션`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `Step Functions, EventBridge, State Machines, Event Bus`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `Step Functions`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/step-functions/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/step-functions/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/instacart`: 기업 블로그 URL
