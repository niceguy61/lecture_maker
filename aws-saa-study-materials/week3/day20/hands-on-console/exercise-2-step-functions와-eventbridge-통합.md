# Exercise 2: Step Functions와 EventBridge 통합

## 🎯 실습 목표
Step Functions를 EventBridge와 통합하여 고급 기능을 구성합니다.
...
- [ ] Exercise 1 완료 (day20-step-functions 생성됨)
...
### Step 1: EventBridge 설정

**Console 경로**: `Services > AWS Services > EventBridge`

1. EventBridge 콘솔로 이동
2. 새 리소스 생성 또는 기존 리소스 선택
3. Step Functions와의 통합 옵션 찾기

---

### Step 2: Step Functions와 연결

**통합 설정**:
1. Step Functions 콘솔로 이동
2. 생성한 리소스 선택 (`day20-step-functions`)
3. "Integrations" 또는 "Connections" 탭 선택
4. "Add integration" 또는 "Connect" 버튼 클릭
5. EventBridge 선택 및 설정
...
4. **Permissions**: 
   - Step FunctionsFullAccess
   - EventBridgeFullAccess
5. **Role name**: `day20-integration-role`
6. "Create role" 클릭
...
**테스트 방법**:
1. Step Functions 콘솔에서 테스트 기능 실행
2. EventBridge에서 데이터 확인
3. 로그 및 메트릭 확인
...
3. 대시보드 이름: `day20-integration-monitoring`
4. 위젯 추가:
   - Step Functions 메트릭
   - EventBridge 메트릭
   - 통합 상태 메트릭
...
2. **AWS Console 이벤트 확인**
   - Console 경로: `Step Functions > Events`
   - 최근 이벤트 및 상태 변경 확인
   - 실패한 작업의 상세 정보 확인
...
**Console 경로**: `Step Functions > Integrations`
...
1. 선택: `day20-integration-monitoring`
2. Delete dashboard
3. 확인: "Delete" 클릭
