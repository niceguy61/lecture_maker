# Hands-On Console: Step Functions 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 20에서 학습한 **Step Functions**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- Step Functions의 핵심 기능을 Console에서 직접 구성
...
- **예상 비용**: $$0.50 - $3.00 (일반적인 실습 비용)
  ...
  - Free Tier 초과: Step Functions
- **소요 시간**: 약 60-90분분
- **권장 리전**: ap-northeast-2 (서울)
- **필요한 권한**: Step FunctionsFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- Step Functions를 중심으로 한 실습 환경
- 주요 구성 요소: Step Functions, EventBridge, State Machines
...
1. **Step Functions 리소스 생성**: AWS Console을 통해 Step Functions를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: Step Functions 생성 및 기본 설정
**목표**: AWS Console을 통해 Step Functions를 생성하고 기본 설정을 구성합니다.
...
1. **Step Functions 생성**
   - Console 경로: Services > Networking > Step Functions
   - 화면 상단의 "Create step" 버튼 클릭
   
2. **기본 설정**
   - **Name**: `day20-step-functions`
     - 명명 규칙: day 번호 + 리소스 유형
     - 예시: `day8-my-bucket`
...
4. **태그 추가** (권장)
   ...
   - 다음 태그 추가:
     | Key | Value | 설명 |
     |-----|-------|------|
     | Name | day20-step-functions | 리소스 식별 |
...
**검증**:
- [ ] step가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
...
### Exercise 2: EventBridge 통합
**목표**: Step Functions와 다른 서비스를 통합합니다.
...
1. **EventBridge 생성**
   - Console 경로: Services > Management > EventBridge
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day20-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **Step Functions와 연결**
   - "Step Functions integration" 또는 "Attach Step Functions" 섹션으로 이동
   - 이전에 생성한 `day20-step-functions` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day20-step-functions-role`
       - 정책: Step FunctionsFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: Step Functions > step-functions > Integrations
     - EventBridge 연결 상태: Connected

**검증**:
- [ ] EventBridge가 정상적으로 생성되었는가?
- [ ] Step Functions와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: Step Functions > step-functions
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day20-Step Functions-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day20-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day20-{metric_name}-alert`
     - Alarm description: Step Functions 성능 알람
...
2. **기능 테스트**
   - Console 경로: {service} > step-functions
   - 기본 기능 테스트:
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day20-Step Functions-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/step/functions
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: Step Functions 생성 완료
- [ ] Exercise 2: EventBridge 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day20-Step Functions-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. EventBridge 삭제
- Console 경로: EventBridge > Resources
- 선택: `day20-monitoring`
...
#### 3. Step Functions 삭제
- Console 경로: Step Functions > Resources
- 선택: `day20-step-functions`
- Actions > Delete
- 확인 옵션:
...
- 검색: `day20` 또는 `Step Functions`
- 선택: 관련 로그 그룹
- Actions > Delete log group(s)
- 확인: "Delete" 버튼 클릭

### 정리 확인 체크리스트
- [ ] 모든 Step Functions 리소스 삭제 완료
- [ ] 모든 EventBridge 리소스 삭제 완료
...
- 확인 항목:
  ...
  - Step Functions 관련 비용: $$0.50 - $3.00
  - 예상 월말 비용: $$0.50 - $3.00
...
### 핵심 개념
1. **Step Functions 아키텍처**: Step Functions의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 20 Theory](../theory.md) - Step Functions 기본 개념
- [Day 19 Theory](../../day19/theory.md) - 사전 학습 내용
- [Day 21 Theory](../../day21/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Instacart의 Step Functions 활용 사례
- [Best Practices](../best-practices.md) - Step Functions 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [Step Functions User Guide](https://docs.aws.amazon.com/step-functions/latest/userguide/)
...
- [Getting Started with Step Functions](https://docs.aws.amazon.com/step-functions/latest/gsg/)
...
- [ ] Step Functions 생성 완료 화면
- [ ] EventBridge 통합 완료 화면
- [ ] CloudWatch 대시보드
- [ ] 테스트 결과 화면
