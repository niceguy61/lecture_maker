# Twitch - 실시간 스트리밍 및 알림 시스템

> **Day 16: SNS (Simple Notification Service) 및 CloudFront**  
> **주요 AWS 서비스**: SNS, CloudFront, Edge Locations, Lambda@Edge
...
- **주요 AWS 서비스**: SNS, CloudFront, Edge Locations, Lambda@Edge
- **사례 출처**: https://aws.amazon.com/architecture/customers/twitch <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
실시간 스트리밍 및 알림 시스템를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > SNS
2. **주요 설정**:
   - Region: day16-resource
...
2. **SNS** → **CloudFront** (Day 16의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **CloudFront** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > SNS > Create Resource

**기본 설정**:
- **Name/ID**: `day16-resource`
...
**Console 경로**: Services > Compute > SNS
...
```yaml
# day16-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Twitch - 실시간 스트리밍 및 알림 시스템 - SNS 구성'

Resources:
  Day16Resource:
    Type: AWS::SNS::Resource
    Properties:
      Name: day16-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_sns" "day16-resource" {
  name = "day16-resource"
  type = "standard"
...
**Console 경로**: CloudWatch > Metrics > SNS
...
**통합 시나리오 설명**:
- Day 16의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 16 (SNS) → Day 1 (Regions)
- Day 1 (Regions) → Day 8 (S3 Buckets)
...
### AWS 공식 문서
- [SNS 사용 설명서](https://docs.aws.amazon.com/sns/latest/userguide/)
- [SNS API 레퍼런스](https://docs.aws.amazon.com/sns/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 실시간 스트리밍 및 알림 시스템](https://aws.amazon.com/architecture/)
- [SNS 베스트 프랙티스](https://docs.aws.amazon.com/sns/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [SNS 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Twitch 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/twitch)
- [AWS re:Invent 발표: Twitch - 실시간 스트리밍 및 알림 시스템](https://reinvent.awsevents.com/)
...
**기본 정보**:
- `16`: 일차 번호 (1-28)
- `SNS (Simple Notification Service) 및 CloudFront`: 일별 학습 주제 제목
- `Twitch`: 기업명 (실제 또는 가상)
- `실시간 스트리밍 및 알림 시스템`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `SNS, CloudFront, Edge Locations, Lambda@Edge`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `SNS`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/sns/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/sns/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/twitch`: 기업 블로그 URL
