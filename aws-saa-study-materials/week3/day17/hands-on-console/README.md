# Hands-On Console: Route 53 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 17에서 학습한 **Route 53**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- Route 53의 핵심 기능을 Console에서 직접 구성
...
- **예상 비용**: $$0.00 (Free Tier 범위 내)
  - Free Tier 범위 내: Route 53
  - Free Tier 초과: 해당 없음
- **소요 시간**: 약 60-90분분
- **권장 리전**: ap-northeast-2 (서울)
- **필요한 권한**: Route 53FullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- Route 53를 중심으로 한 실습 환경
- 주요 구성 요소: Route 53, DNS, Health Checks
...
1. **Route 53 리소스 생성**: AWS Console을 통해 Route 53를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: Route 53 생성 및 기본 설정
**목표**: AWS Console을 통해 Route 53를 생성하고 기본 설정을 구성합니다.
...
1. **Route 53 생성**
   - Console 경로: Services > Networking > Route 53
   - 화면 상단의 "Create route" 버튼 클릭
...
**검증**:
- [ ] route가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
...
1. **DNS 생성**
   - Console 경로: Services > Management > DNS
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day17-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **Route 53와 연결**
   - "Route 53 integration" 또는 "Attach Route 53" 섹션으로 이동
   - 이전에 생성한 `day17-route-53` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day17-route-53-role`
       - 정책: Route 53FullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: Route 53 > route-53 > Integrations
     - DNS 연결 상태: Connected

**검증**:
- [ ] DNS가 정상적으로 생성되었는가?
- [ ] Route 53와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: Route 53 > route-53
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day17-Route 53-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day17-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day17-{metric_name}-alert`
     - Alarm description: Route 53 성능 알람
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day17-Route 53-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/route/53
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: Route 53 생성 완료
- [ ] Exercise 2: DNS 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day17-Route 53-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. DNS 삭제
- Console 경로: DNS > Resources
- 선택: `day17-monitoring`
...
- 검색: `day17` 또는 `Route 53`
- 선택: 관련 로그 그룹
- Actions > Delete log group(s)
- 확인: "Delete" 버튼 클릭

### 정리 확인 체크리스트
- [ ] 모든 Route 53 리소스 삭제 완료
- [ ] 모든 DNS 리소스 삭제 완료
...
### 핵심 개념
1. **Route 53 아키텍처**: Route 53의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 17 Theory](../theory.md) - Route 53 기본 개념
- [Day 16 Theory](../../day16/theory.md) - 사전 학습 내용
- [Day 18 Theory](../../day18/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Coursera의 Route 53 활용 사례
- [Best Practices](../best-practices.md) - Route 53 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [Route 53 User Guide](https://docs.aws.amazon.com/route-53/latest/userguide/)
- [Route 53 API Reference](https://docs.aws.amazon.com/route-53/latest/APIReference/)
- [AWS Console User Guide](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/)
- [Getting Started with Route 53](https://docs.aws.amazon.com/route-53/latest/gsg/)
