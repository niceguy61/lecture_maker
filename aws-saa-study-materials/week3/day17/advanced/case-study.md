# Coursera - 글로벌 교육 플랫폼의 DNS 및 트래픽 라우팅

> **Day 17: Route 53**  
> **주요 AWS 서비스**: Route 53, DNS, Health Checks, Routing Policies
...
- **주요 AWS 서비스**: Route 53, DNS, Health Checks, Routing Policies
- **사례 출처**: https://aws.amazon.com/architecture/customers/coursera <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
글로벌 교육 플랫폼의 DNS 및 트래픽 라우팅를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > Route 53
2. **주요 설정**:
   - Region: day17-resource
...
sequenceDiagram
    ...
    participant Service as Route 53
...
**Console 경로**: Services > Compute > Route 53 > Create Resource

**기본 설정**:
- **Name/ID**: `day17-resource`
...
**Console 경로**: Services > Compute > Route 53

**연결 설정**:
1. Route 53에서 생성한 리소스 선택
...
```yaml
# day17-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Coursera - 글로벌 교육 플랫폼의 DNS 및 트래픽 라우팅 - Route 53 구성'

Resources:
  Day17Resource:
    Type: AWS::Route53::Resource
    Properties:
      Name: day17-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_route_53" "day17-resource" {
  name = "day17-resource"
  type = "standard"
...
**Console 경로**: CloudWatch > Metrics > Route53
...
#### Day 13: ALB
**연계 방식**: Day 13의 ELB (Elastic Load Balancing)와 연계
...
**통합 시나리오 설명**:
- Day 17의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 17 (Route 53) → Day 1 (Regions)
- Day 1 (Regions) → Day 13 (ALB)
...
### AWS 공식 문서
- [Route 53 사용 설명서](https://docs.aws.amazon.com/route-53/latest/userguide/)
- [Route 53 API 레퍼런스](https://docs.aws.amazon.com/route-53/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 글로벌 교육 플랫폼의 DNS 및 트래픽 라우팅](https://aws.amazon.com/architecture/)
- [Route 53 베스트 프랙티스](https://docs.aws.amazon.com/route-53/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [Route 53 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Coursera 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/coursera)
- [AWS re:Invent 발표: Coursera - 글로벌 교육 플랫폼의 DNS 및 트래픽 라우팅](https://reinvent.awsevents.com/)
...
**기본 정보**:
- `17`: 일차 번호 (1-28)
- `Route 53`: 일별 학습 주제 제목
- `Coursera`: 기업명 (실제 또는 가상)
- `글로벌 교육 플랫폼의 DNS 및 트래픽 라우팅`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `Route 53, DNS, Health Checks, Routing Policies`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `Route 53`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/route-53/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/route-53/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/coursera`: 기업 블로그 URL
