# Hands-On Console: Lambda 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 18에서 학습한 **Lambda**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- Lambda의 핵심 기능을 Console에서 직접 구성
...
- **필요한 권한**: LambdaFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- Lambda를 중심으로 한 실습 환경
- 주요 구성 요소: Lambda, Lambda Layers, Lambda@Edge
...
1. **Lambda 리소스 생성**: AWS Console을 통해 Lambda를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: Lambda 생성 및 기본 설정
**목표**: AWS Console을 통해 Lambda를 생성하고 기본 설정을 구성합니다.
...
1. **Lambda 생성**
   - Console 경로: Services > Networking > Lambda
   - 화면 상단의 "Create lambda" 버튼 클릭
...
**검증**:
- [ ] lambda가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
...
### Exercise 2: Lambda Layers 통합
**목표**: Lambda와 다른 서비스를 통합합니다.
...
1. **Lambda Layers 생성**
   - Console 경로: Services > Management > Lambda Layers
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day18-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **Lambda와 연결**
   - "Lambda integration" 또는 "Attach Lambda" 섹션으로 이동
   - 이전에 생성한 `day18-lambda` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day18-lambda-role`
       - 정책: LambdaFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: Lambda > lambda > Integrations
     - Lambda Layers 연결 상태: Connected

**검증**:
- [ ] Lambda Layers가 정상적으로 생성되었는가?
- [ ] Lambda와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: Lambda > lambda
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day18-Lambda-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day18-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day18-{metric_name}-alert`
     - Alarm description: Lambda 성능 알람
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day18-Lambda-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/lambda
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: Lambda 생성 완료
- [ ] Exercise 2: Lambda Layers 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day18-Lambda-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. Lambda Layers 삭제
- Console 경로: Lambda Layers > Resources
- 선택: `day18-monitoring`
...
### 핵심 개념
1. **Lambda 아키텍처**: Lambda의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 18 Theory](../theory.md) - Lambda 기본 개념
- [Day 17 Theory](../../day17/theory.md) - 사전 학습 내용
- [Day 19 Theory](../../day19/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Coca-Cola의 Lambda 활용 사례
- [Best Practices](../best-practices.md) - Lambda 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [Lambda User Guide](https://docs.aws.amazon.com/lambda/latest/userguide/)
...
- [Getting Started with Lambda](https://docs.aws.amazon.com/lambda/latest/gsg/)
