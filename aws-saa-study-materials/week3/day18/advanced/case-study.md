# Coca-Cola - 자판기 IoT 데이터 처리를 위한 서버리스 아키텍처

> **Day 18: Lambda (서버리스 컴퓨팅)**  
> **주요 AWS 서비스**: Lambda, Lambda Layers, Lambda@Edge, Event Sources
...
- **주요 AWS 서비스**: Lambda, Lambda Layers, Lambda@Edge, Event Sources
- **사례 출처**: https://aws.amazon.com/architecture/customers/coca-cola <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
자판기 IoT 데이터 처리를 위한 서버리스 아키텍처를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > Lambda
2. **주요 설정**:
   - Region: day18-resource
...
2. **Lambda** → **Lambda Layers** (Day 18의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **Lambda Layers** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > Lambda > Create Resource

**기본 설정**:
- **Name/ID**: `day18-resource`
...
```yaml
# day18-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Coca-Cola - 자판기 IoT 데이터 처리를 위한 서버리스 아키텍처 - Lambda 구성'

Resources:
  Day18Resource:
    Type: AWS::Lambda::Resource
    Properties:
      Name: day18-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_lambda" "day18-resource" {
  name = "day18-resource"
  type = "standard"
  
  tags = {
    Project     = "day18-project"
...
**통합 시나리오 설명**:
- Day 18의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 18 (Lambda) → Day 11 (DynamoDB)
- Day 11 (DynamoDB) → Day 15 (SQS)
...
### AWS 공식 문서
- [Lambda 사용 설명서](https://docs.aws.amazon.com/lambda/latest/userguide/)
- [Lambda API 레퍼런스](https://docs.aws.amazon.com/lambda/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 자판기 IoT 데이터 처리를 위한 서버리스 아키텍처](https://aws.amazon.com/architecture/)
- [Lambda 베스트 프랙티스](https://docs.aws.amazon.com/lambda/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [Lambda 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Coca-Cola 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/coca-cola)
- [AWS re:Invent 발표: Coca-Cola - 자판기 IoT 데이터 처리를 위한 서버리스 아키텍처](https://reinvent.awsevents.com/)
...
**기본 정보**:
- `18`: 일차 번호 (1-28)
- `Lambda (서버리스 컴퓨팅)`: 일별 학습 주제 제목
- `Coca-Cola`: 기업명 (실제 또는 가상)
- `자판기 IoT 데이터 처리를 위한 서버리스 아키텍처`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `Lambda, Lambda Layers, Lambda@Edge, Event Sources`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `Lambda`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/lambda/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/lambda/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/coca-cola`: 기업 블로그 URL
