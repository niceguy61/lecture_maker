...
- **주요 AWS 서비스**: VPC, Subnets, Route Tables, Internet Gateway
- **사례 출처**: https://aws.amazon.com/architecture/customers/slack <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
엔터프라이즈급 네트워크 격리 및 보안를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > VPC
2. **주요 설정**:
   - Region: day5-resource
   - Instance Type / Configuration: Standard
   - Auto Scaling: 활성화

**다른 서비스와의 연계**:
- **NAT Gateway** (Day 6): Day 6의 VPC 고급 기능와 연계
- **IAM Users** (Day 2): Day 2의 IAM (Identity and Access Management)와 연계

#### Subnets

**역할**: 네트워크 세그먼테이션 및 리소스 격리

**구성 방법**:
- 퍼블릭/프라이빗 서브넷 생성 및 CIDR 할당
...
2. **VPC** → **Subnets** (Day 5의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **Subnets** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > VPC > Create Resource

**기본 설정**:
- **Name/ID**: `day5-resource`
...
**Console 경로**: Services > Compute > VPC
...
```yaml
# day5-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Slack - 엔터프라이즈급 네트워크 격리 및 보안 - VPC 구성'

Resources:
  Day5Resource:
    Type: AWS::VPC::Resource
    Properties:
      Name: day5-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_vpc" "day5-resource" {
  name = "day5-resource"
  type = "standard"
  
  tags = {
    Project     = "day5-project"
...
**Console 경로**: CloudWatch > Metrics > VPC
...
**통합 시나리오 설명**:
- Day 5의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 5 (VPC) → Day 6 (NAT Gateway)
- Day 6 (NAT Gateway) → Day 2 (IAM Users)
...
- [VPC API 레퍼런스](https://docs.aws.amazon.com/vpc/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 엔터프라이즈급 네트워크 격리 및 보안](https://aws.amazon.com/architecture/)
- [VPC 베스트 프랙티스](https://docs.aws.amazon.com/vpc/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [VPC 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Slack 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/slack)
- [AWS re:Invent 발표: Slack - 엔터프라이즈급 네트워크 격리 및 보안](https://reinvent.awsevents.com/)
...
**기본 정보**:
- `5`: 일차 번호 (1-28)
- `VPC (Virtual Private Cloud) 기초`: 일별 학습 주제 제목
- `Slack`: 기업명 (실제 또는 가상)
- `엔터프라이즈급 네트워크 격리 및 보안`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `VPC, Subnets, Route Tables, Internet Gateway`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `VPC`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/vpc/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/vpc/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/slack`: 기업 블로그 URL
