# Airbnb - 대규모 조직의 IAM 보안 아키텍처

> **Day 2: IAM (Identity and Access Management)**  
> **주요 AWS 서비스**: IAM Users, Groups, Roles, Policies, MFA
...
- **주요 AWS 서비스**: IAM Users, Groups, Roles, Policies, MFA
- **사례 출처**: https://aws.amazon.com/architecture/customers/airbnb <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
대규모 조직의 IAM 보안 아키텍처를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > IAM Users
2. **주요 설정**:
   - Region: day2-resource
...
sequenceDiagram
    ...
    participant Service as IAM Users
...
2. **IAM Users** → **Groups** (Day 2의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **Groups** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > IAM Users > Create Resource

**기본 설정**:
- **Name/ID**: `day2-resource`
...
**Console 경로**: Services > Compute > IAM Users

**연결 설정**:
1. IAM Users에서 생성한 리소스 선택
...
```yaml
# day2-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Airbnb - 대규모 조직의 IAM 보안 아키텍처 - IAM Users 구성'

Resources:
  Day2Resource:
    Type: AWS::IAMUsers::Resource
    Properties:
      Name: day2-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_iam_users" "day2-resource" {
  name = "day2-resource"
  type = "standard"
...
**Console 경로**: CloudWatch > Metrics > IAMUsers
...
**통합 시나리오 설명**:
- Day 2의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 2 (IAM Users) → Day 3 (EC2 Instances)
- Day 3 (EC2 Instances) → Day 5 (VPC)
...
### AWS 공식 문서
- [IAM Users 사용 설명서](https://docs.aws.amazon.com/iam-users/latest/userguide/)
- [IAM Users API 레퍼런스](https://docs.aws.amazon.com/iam-users/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 대규모 조직의 IAM 보안 아키텍처](https://aws.amazon.com/architecture/)
- [IAM Users 베스트 프랙티스](https://docs.aws.amazon.com/iam-users/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [IAM Users 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Airbnb 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/airbnb)
- [AWS re:Invent 발표: Airbnb - 대규모 조직의 IAM 보안 아키텍처](https://reinvent.awsevents.com/)
...
**기본 정보**:
- `2`: 일차 번호 (1-28)
- `IAM (Identity and Access Management)`: 일별 학습 주제 제목
- `Airbnb`: 기업명 (실제 또는 가상)
- `대규모 조직의 IAM 보안 아키텍처`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `IAM Users, Groups, Roles, Policies, MFA`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `IAM Users`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/iam-users/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/iam-users/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/airbnb`: 기업 블로그 URL
