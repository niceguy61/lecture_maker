# Hands-On Console: Regions 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 1에서 학습한 **Regions**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- Regions의 핵심 기능을 Console에서 직접 구성
...
- **예상 비용**: $$0.00 (Free Tier 범위 내)
  - Free Tier 범위 내: Regions
  - Free Tier 초과: 해당 없음
- **소요 시간**: 약 20-30분분
- **권장 리전**: ap-northeast-2 (서울)
- **필요한 권한**: RegionsFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- Regions를 중심으로 한 실습 환경
- 주요 구성 요소: Regions, Availability Zones, Edge Locations
...
1. **Regions 리소스 생성**: AWS Console을 통해 Regions를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: Regions 생성 및 기본 설정
**목표**: AWS Console을 통해 Regions를 생성하고 기본 설정을 구성합니다.
...
1. **Regions 생성**
   - Console 경로: Services > Networking > Regions
   - 화면 상단의 "Create regions" 버튼 클릭
...
5. **생성 확인**
   - 모든 설정 검토
   - "Create regions" 버튼 클릭
   - 생성 완료까지 대기 (약 2-5분)
   - 상태 확인: 리소스 목록에서 상태 컬럼 확인

**검증**:
- [ ] regions가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
...
### Exercise 2: Availability Zones 통합
**목표**: Regions와 다른 서비스를 통합합니다.

**예상 소요 시간**: 10-15분

**단계**:

1. **Availability Zones 생성**
   - Console 경로: Services > Management > Availability Zones
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day1-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **Regions와 연결**
   - "Regions integration" 또는 "Attach Regions" 섹션으로 이동
   - 이전에 생성한 `day1-regions` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day1-regions-role`
       - 정책: RegionsFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: Regions > regions > Integrations
     - Availability Zones 연결 상태: Connected

**검증**:
- [ ] Availability Zones가 정상적으로 생성되었는가?
- [ ] Regions와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: Regions > regions
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day1-Regions-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day1-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day1-{metric_name}-alert`
     - Alarm description: Regions 성능 알람
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day1-Regions-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/regions
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: Regions 생성 완료
- [ ] Exercise 2: Availability Zones 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day1-Regions-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. Availability Zones 삭제
- Console 경로: Availability Zones > Resources
- 선택: `day1-monitoring`
...
### 정리 확인 체크리스트
- [ ] 모든 Regions 리소스 삭제 완료
- [ ] 모든 Availability Zones 리소스 삭제 완료
...
### 핵심 개념
1. **Regions 아키텍처**: Regions의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 1 Theory](../theory.md) - Regions 기본 개념
- [Day 1 Theory](../../day1/theory.md) - 사전 학습 내용
- [Day 2 Theory](../../day2/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Netflix의 Regions 활용 사례
- [Best Practices](../best-practices.md) - Regions 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [Regions User Guide](https://docs.aws.amazon.com/regions/latest/userguide/)
- [Regions API Reference](https://docs.aws.amazon.com/regions/latest/APIReference/)
- [AWS Console User Guide](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/)
- [Getting Started with Regions](https://docs.aws.amazon.com/regions/latest/gsg/)
