# Hands-On Console: EC2 Instances 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 3에서 학습한 **EC2 Instances**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- EC2 Instances의 핵심 기능을 Console에서 직접 구성
...
- **예상 비용**: $$0.50 - $2.00 (t2.micro/t3.micro 인스턴스 사용 시)
  ...
  - Free Tier 초과: EC2 Instances
- **소요 시간**: 약 30-45분분
- **권장 리전**: ap-northeast-2 (서울)
- **필요한 권한**: EC2 InstancesFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- EC2 Instances를 중심으로 한 실습 환경
- 주요 구성 요소: EC2 Instances, Instance Types, AMI
...
1. **EC2 Instances 리소스 생성**: AWS Console을 통해 EC2 Instances를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: EC2 Instances 생성 및 기본 설정
**목표**: AWS Console을 통해 EC2 Instances를 생성하고 기본 설정을 구성합니다.
...
1. **EC2 Instances 생성**
   - Console 경로: Services > Compute > EC2 Instances
   - 화면 상단의 "Create ec2" 버튼 클릭
   
2. **기본 설정**
   - **Name**: `day3-ec2-instances`
     - 명명 규칙: day 번호 + 리소스 유형
     - 예시: `day8-my-bucket`
...
4. **태그 추가** (권장)
   ...
   - 다음 태그 추가:
     | Key | Value | 설명 |
     |-----|-------|------|
     | Name | day3-ec2-instances | 리소스 식별 |
...
### Exercise 2: Instance Types 통합
**목표**: EC2 Instances와 다른 서비스를 통합합니다.
...
1. **Instance Types 생성**
   - Console 경로: Services > Management > Instance Types
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day3-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **EC2 Instances와 연결**
   - "EC2 Instances integration" 또는 "Attach EC2 Instances" 섹션으로 이동
   - 이전에 생성한 `day3-ec2-instances` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day3-ec2-instances-role`
       - 정책: EC2 InstancesFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: EC2 Instances > ec2-instances > Integrations
     - Instance Types 연결 상태: Connected

**검증**:
- [ ] Instance Types가 정상적으로 생성되었는가?
- [ ] EC2 Instances와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: EC2 Instances > ec2-instances
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day3-EC2 Instances-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day3-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day3-{metric_name}-alert`
     - Alarm description: EC2 Instances 성능 알람
...
2. **기능 테스트**
   - Console 경로: {service} > ec2-instances
   - 기본 기능 테스트:
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day3-EC2 Instances-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/ec2/instances
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: EC2 Instances 생성 완료
- [ ] Exercise 2: Instance Types 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day3-EC2 Instances-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. Instance Types 삭제
- Console 경로: Instance Types > Resources
- 선택: `day3-monitoring`
...
#### 3. EC2 Instances 삭제
- Console 경로: EC2 Instances > Resources
- 선택: `day3-ec2-instances`
- Actions > Delete
- 확인 옵션:
...
- 검색: `day3` 또는 `EC2 Instances`
- 선택: 관련 로그 그룹
- Actions > Delete log group(s)
- 확인: "Delete" 버튼 클릭

### 정리 확인 체크리스트
- [ ] 모든 EC2 Instances 리소스 삭제 완료
- [ ] 모든 Instance Types 리소스 삭제 완료
...
### 핵심 개념
1. **EC2 Instances 아키텍처**: EC2 Instances의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 3 Theory](../theory.md) - EC2 Instances 기본 개념
- [Day 2 Theory](../../day2/theory.md) - 사전 학습 내용
- [Day 4 Theory](../../day4/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Spotify의 EC2 Instances 활용 사례
- [Best Practices](../best-practices.md) - EC2 Instances 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [EC2 Instances User Guide](https://docs.aws.amazon.com/ec2-instances/latest/userguide/)
- [EC2 Instances API Reference](https://docs.aws.amazon.com/ec2-instances/latest/APIReference/)
- [AWS Console User Guide](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/)
- [Getting Started with EC2 Instances](https://docs.aws.amazon.com/ec2-instances/latest/gsg/)
...
- [ ] EC2 Instances 생성 완료 화면
- [ ] Instance Types 통합 완료 화면
- [ ] CloudWatch 대시보드
- [ ] 테스트 결과 화면
