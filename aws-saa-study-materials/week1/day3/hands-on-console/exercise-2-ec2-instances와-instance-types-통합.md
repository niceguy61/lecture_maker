# Exercise 2: EC2 Instances와 Instance Types 통합

## 🎯 실습 목표
EC2 Instances를 Instance Types와 통합하여 고급 기능을 구성합니다.
...
- [ ] Exercise 1 완료 (day3-ec2-instances 생성됨)
...
graph TB
    ...
    subgraph "AWS Console - Exercise 2"
        Service1[EC2 Instances]
        Service2[Instance Types]
...
**구성 요소**:
- **EC2 Instances**: 메인 리소스
- **Instance Types**: 통합 서비스
...
### Step 1: Instance Types 설정

**Console 경로**: `Services > AWS Services > Instance Types`

1. Instance Types 콘솔로 이동
2. 새 리소스 생성 또는 기존 리소스 선택
3. EC2 Instances와의 통합 옵션 찾기

---

### Step 2: EC2 Instances와 연결

**통합 설정**:
1. EC2 Instances 콘솔로 이동
2. 생성한 리소스 선택 (`day3-ec2-instances`)
3. "Integrations" 또는 "Connections" 탭 선택
4. "Add integration" 또는 "Connect" 버튼 클릭
5. Instance Types 선택 및 설정
...
3. **Use case**: EC2 Instances
4. **Permissions**: 
   - EC2 InstancesFullAccess
   - Instance TypesFullAccess
5. **Role name**: `day3-integration-role`
6. "Create role" 클릭
...
**테스트 방법**:
1. EC2 Instances 콘솔에서 테스트 기능 실행
2. Instance Types에서 데이터 확인
3. 로그 및 메트릭 확인
...
3. 대시보드 이름: `day3-integration-monitoring`
4. 위젯 추가:
   - EC2 Instances 메트릭
   - Instance Types 메트릭
   - 통합 상태 메트릭
...
2. **AWS Console 이벤트 확인**
   - Console 경로: `EC2 Instances > Events`
   - 최근 이벤트 및 상태 변경 확인
   - 실패한 작업의 상세 정보 확인
...
**Console 경로**: `EC2 Instances > Integrations`
...
1. 선택: `day3-integration-monitoring`
2. Delete dashboard
3. 확인: "Delete" 클릭
