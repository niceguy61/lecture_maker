# Hands-On Console: Auto Scaling 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 4에서 학습한 **Auto Scaling**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- Auto Scaling의 핵심 기능을 Console에서 직접 구성
...
- **예상 비용**: $$0.50 - $2.00 (t2.micro/t3.micro 인스턴스 사용 시)
  ...
  - Free Tier 초과: Auto Scaling
- **소요 시간**: 약 30-45분분
- **권장 리전**: ap-northeast-2 (서울)
- **필요한 권한**: Auto ScalingFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- Auto Scaling를 중심으로 한 실습 환경
- 주요 구성 요소: Auto Scaling, Load Balancing, Placement Groups
...
1. **Auto Scaling 리소스 생성**: AWS Console을 통해 Auto Scaling를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: Auto Scaling 생성 및 기본 설정
**목표**: AWS Console을 통해 Auto Scaling를 생성하고 기본 설정을 구성합니다.
...
1. **Auto Scaling 생성**
   - Console 경로: Services > Networking > Auto Scaling
   - 화면 상단의 "Create auto" 버튼 클릭
   
2. **기본 설정**
   - **Name**: `day4-auto-scaling`
     - 명명 규칙: day 번호 + 리소스 유형
     - 예시: `day8-my-bucket`
...
**검증**:
- [ ] auto가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
...
### Exercise 2: Load Balancing 통합
**목표**: Auto Scaling와 다른 서비스를 통합합니다.
...
1. **Load Balancing 생성**
   - Console 경로: Services > Management > Load Balancing
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day4-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **Auto Scaling와 연결**
   - "Auto Scaling integration" 또는 "Attach Auto Scaling" 섹션으로 이동
   - 이전에 생성한 `day4-auto-scaling` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day4-auto-scaling-role`
       - 정책: Auto ScalingFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: Auto Scaling > auto-scaling > Integrations
     - Load Balancing 연결 상태: Connected

**검증**:
- [ ] Load Balancing가 정상적으로 생성되었는가?
- [ ] Auto Scaling와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: Auto Scaling > auto-scaling
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day4-Auto Scaling-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day4-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day4-{metric_name}-alert`
     - Alarm description: Auto Scaling 성능 알람
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day4-Auto Scaling-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/auto/scaling
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: Auto Scaling 생성 완료
- [ ] Exercise 2: Load Balancing 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day4-Auto Scaling-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. Load Balancing 삭제
- Console 경로: Load Balancing > Resources
- 선택: `day4-monitoring`
...
#### 3. Auto Scaling 삭제
- Console 경로: Auto Scaling > Resources
- 선택: `day4-auto-scaling`
- Actions > Delete
- 확인 옵션:
...
- 검색: `day4` 또는 `Auto Scaling`
- 선택: 관련 로그 그룹
- Actions > Delete log group(s)
- 확인: "Delete" 버튼 클릭

### 정리 확인 체크리스트
- [ ] 모든 Auto Scaling 리소스 삭제 완료
- [ ] 모든 Load Balancing 리소스 삭제 완료
...
### 핵심 개념
1. **Auto Scaling 아키텍처**: Auto Scaling의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 4 Theory](../theory.md) - Auto Scaling 기본 개념
- [Day 3 Theory](../../day3/theory.md) - 사전 학습 내용
- [Day 5 Theory](../../day5/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Uber의 Auto Scaling 활용 사례
- [Best Practices](../best-practices.md) - Auto Scaling 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [Auto Scaling User Guide](https://docs.aws.amazon.com/auto-scaling/latest/userguide/)
- [Auto Scaling API Reference](https://docs.aws.amazon.com/auto-scaling/latest/APIReference/)
- [AWS Console User Guide](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/)
- [Getting Started with Auto Scaling](https://docs.aws.amazon.com/auto-scaling/latest/gsg/)
...
- [ ] Auto Scaling 생성 완료 화면
- [ ] Load Balancing 통합 완료 화면
- [ ] CloudWatch 대시보드
- [ ] 테스트 결과 화면
