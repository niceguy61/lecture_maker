# Exercise 2: Auto Scaling와 Load Balancing 통합

## 🎯 실습 목표
Auto Scaling를 Load Balancing와 통합하여 고급 기능을 구성합니다.
...
- [ ] Exercise 1 완료 (day4-auto-scaling 생성됨)
...
graph TB
    ...
    subgraph "AWS Console - Exercise 2"
        Service1[Auto Scaling]
        Service2[Load Balancing]
...
### Step 1: Load Balancing 설정

**Console 경로**: `Services > AWS Services > Load Balancing`

1. Load Balancing 콘솔로 이동
2. 새 리소스 생성 또는 기존 리소스 선택
3. Auto Scaling와의 통합 옵션 찾기

---

### Step 2: Auto Scaling와 연결

**통합 설정**:
1. Auto Scaling 콘솔로 이동
2. 생성한 리소스 선택 (`day4-auto-scaling`)
3. "Integrations" 또는 "Connections" 탭 선택
4. "Add integration" 또는 "Connect" 버튼 클릭
5. Load Balancing 선택 및 설정
...
3. **Use case**: Auto Scaling
4. **Permissions**: 
   - Auto ScalingFullAccess
   - Load BalancingFullAccess
5. **Role name**: `day4-integration-role`
6. "Create role" 클릭
...
**테스트 방법**:
1. Auto Scaling 콘솔에서 테스트 기능 실행
2. Load Balancing에서 데이터 확인
3. 로그 및 메트릭 확인
...
3. 대시보드 이름: `day4-integration-monitoring`
4. 위젯 추가:
   - Auto Scaling 메트릭
   - Load Balancing 메트릭
   - 통합 상태 메트릭
...
2. **AWS Console 이벤트 확인**
   - Console 경로: `Auto Scaling > Events`
   - 최근 이벤트 및 상태 변경 확인
   - 실패한 작업의 상세 정보 확인
...
**Console 경로**: `Auto Scaling > Integrations`
...
1. 선택: `day4-integration-monitoring`
2. Delete dashboard
3. 확인: "Delete" 클릭
