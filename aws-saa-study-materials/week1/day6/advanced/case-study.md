# Capital One - 금융 서비스를 위한 하이브리드 클라우드 네트워킹

> **Day 6: VPC 고급 기능**  
> **주요 AWS 서비스**: NAT Gateway, VPC Peering, VPN, Direct Connect
...
- **주요 AWS 서비스**: NAT Gateway, VPC Peering, VPN, Direct Connect
- **사례 출처**: https://aws.amazon.com/architecture/customers/capital-one <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
금융 서비스를 위한 하이브리드 클라우드 네트워킹를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > NAT Gateway
2. **주요 설정**:
   - Region: day6-resource
...
sequenceDiagram
    ...
    participant Service as NAT Gateway
...
**플로우 설명**:
1. **사용자 요청** → NAT Gateway
   - 사용자 요청을 받아 처리 시작
   
2. **NAT Gateway** → **VPC Peering** (Day 6의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **VPC Peering** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > NAT Gateway > Create Resource

**기본 설정**:
- **Name/ID**: `day6-resource`
...
**Console 경로**: Services > Compute > NAT Gateway

**연결 설정**:
1. NAT Gateway에서 생성한 리소스 선택
...
```yaml
# day6-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Capital One - 금융 서비스를 위한 하이브리드 클라우드 네트워킹 - NAT Gateway 구성'

Resources:
  Day6Resource:
    Type: AWS::NATGateway::Resource
    Properties:
      Name: day6-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_nat_gateway" "day6-resource" {
  name = "day6-resource"
  type = "standard"
  
  tags = {
    Project     = "day6-project"
...
**Console 경로**: CloudWatch > Metrics > NATGateway
...
**통합 시나리오 설명**:
- Day 6의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 6 (NAT Gateway) → Day 5 (VPC)
- Day 5 (VPC) → Day 23 (CloudTrail)
...
### AWS 공식 문서
- [NAT Gateway 사용 설명서](https://docs.aws.amazon.com/nat-gateway/latest/userguide/)
- [NAT Gateway API 레퍼런스](https://docs.aws.amazon.com/nat-gateway/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 금융 서비스를 위한 하이브리드 클라우드 네트워킹](https://aws.amazon.com/architecture/)
- [NAT Gateway 베스트 프랙티스](https://docs.aws.amazon.com/nat-gateway/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [NAT Gateway 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Capital One 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/capital-one)
- [AWS re:Invent 발표: Capital One - 금융 서비스를 위한 하이브리드 클라우드 네트워킹](https://reinvent.awsevents.com/)
...
### 1. NAT Gateway의 실제 활용 방법
- NAT Gateway의 핵심 기능 이해
- 실제 프로덕션 환경 구성 방법
- 모범 사례 및 안티 패턴
...
**기본 정보**:
- `6`: 일차 번호 (1-28)
- `VPC 고급 기능`: 일별 학습 주제 제목
- `Capital One`: 기업명 (실제 또는 가상)
- `금융 서비스를 위한 하이브리드 클라우드 네트워킹`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `NAT Gateway, VPC Peering, VPN, Direct Connect`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `NAT Gateway`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/nat-gateway/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/nat-gateway/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/capital-one`: 기업 블로그 URL
