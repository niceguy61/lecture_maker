# Hands-On Console: NAT Gateway 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 6에서 학습한 **NAT Gateway**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- NAT Gateway의 핵심 기능을 Console에서 직접 구성
...
### 실습 환경 정보
- **예상 비용**: $$0.00 (Free Tier 범위 내)
  - Free Tier 범위 내: NAT Gateway
  - Free Tier 초과: 해당 없음
- **소요 시간**: 약 45-60분분
- **권장 리전**: ap-northeast-2 (서울)
- **필요한 권한**: NAT GatewayFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- NAT Gateway를 중심으로 한 실습 환경
- 주요 구성 요소: NAT Gateway, VPC Peering, VPN
...
1. **NAT Gateway 리소스 생성**: AWS Console을 통해 NAT Gateway를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: NAT Gateway 생성 및 기본 설정
**목표**: AWS Console을 통해 NAT Gateway를 생성하고 기본 설정을 구성합니다.
...
1. **NAT Gateway 생성**
   - Console 경로: Services > Networking > NAT Gateway
   - 화면 상단의 "Create nat" 버튼 클릭
...
### Exercise 2: VPC Peering 통합
**목표**: NAT Gateway와 다른 서비스를 통합합니다.
...
1. **VPC Peering 생성**
   - Console 경로: Services > Management > VPC Peering
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day6-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **NAT Gateway와 연결**
   - "NAT Gateway integration" 또는 "Attach NAT Gateway" 섹션으로 이동
   - 이전에 생성한 `day6-nat-gateway` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day6-nat-gateway-role`
       - 정책: NAT GatewayFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: NAT Gateway > nat-gateway > Integrations
     - VPC Peering 연결 상태: Connected

**검증**:
- [ ] VPC Peering가 정상적으로 생성되었는가?
- [ ] NAT Gateway와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: NAT Gateway > nat-gateway
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day6-NAT Gateway-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day6-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day6-{metric_name}-alert`
     - Alarm description: NAT Gateway 성능 알람
...
2. **기능 테스트**
   - Console 경로: {service} > nat-gateway
   - 기본 기능 테스트:
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day6-NAT Gateway-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/nat/gateway
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: NAT Gateway 생성 완료
- [ ] Exercise 2: VPC Peering 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day6-NAT Gateway-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. VPC Peering 삭제
- Console 경로: VPC Peering > Resources
- 선택: `day6-monitoring`
...
#### 3. NAT Gateway 삭제
- Console 경로: NAT Gateway > Resources
- 선택: `day6-nat-gateway`
- Actions > Delete
- 확인 옵션:
...
- 검색: `day6` 또는 `NAT Gateway`
- 선택: 관련 로그 그룹
- Actions > Delete log group(s)
- 확인: "Delete" 버튼 클릭

### 정리 확인 체크리스트
- [ ] 모든 NAT Gateway 리소스 삭제 완료
- [ ] 모든 VPC Peering 리소스 삭제 완료
...
### 핵심 개념
1. **NAT Gateway 아키텍처**: NAT Gateway의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 6 Theory](../theory.md) - NAT Gateway 기본 개념
- [Day 5 Theory](../../day5/theory.md) - 사전 학습 내용
- [Day 7 Theory](../../day7/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Capital One의 NAT Gateway 활용 사례
- [Best Practices](../best-practices.md) - NAT Gateway 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [NAT Gateway User Guide](https://docs.aws.amazon.com/nat-gateway/latest/userguide/)
- [NAT Gateway API Reference](https://docs.aws.amazon.com/nat-gateway/latest/APIReference/)
- [AWS Console User Guide](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/)
- [Getting Started with NAT Gateway](https://docs.aws.amazon.com/nat-gateway/latest/gsg/)
...
- [ ] NAT Gateway 생성 완료 화면
- [ ] VPC Peering 통합 완료 화면
- [ ] CloudWatch 대시보드
- [ ] 테스트 결과 화면
