# Exercise 2: NAT Gateway와 VPC Peering 통합

## 🎯 실습 목표
NAT Gateway를 VPC Peering와 통합하여 고급 기능을 구성합니다.
...
- [ ] Exercise 1 완료 (day6-nat-gateway 생성됨)
...
graph TB
    ...
    subgraph "AWS Console - Exercise 2"
        Service1[NAT Gateway]
        Service2[VPC Peering]
...
### Step 1: VPC Peering 설정

**Console 경로**: `Services > Networking > VPC Peering`

1. VPC Peering 콘솔로 이동
2. 새 리소스 생성 또는 기존 리소스 선택
3. NAT Gateway와의 통합 옵션 찾기

---

### Step 2: NAT Gateway와 연결

**통합 설정**:
1. NAT Gateway 콘솔로 이동
2. 생성한 리소스 선택 (`day6-nat-gateway`)
3. "Integrations" 또는 "Connections" 탭 선택
4. "Add integration" 또는 "Connect" 버튼 클릭
5. VPC Peering 선택 및 설정
...
3. **Use case**: NAT Gateway
4. **Permissions**: 
   - NAT GatewayFullAccess
   - VPC PeeringFullAccess
5. **Role name**: `day6-integration-role`
6. "Create role" 클릭
...
**테스트 방법**:
1. NAT Gateway 콘솔에서 테스트 기능 실행
2. VPC Peering에서 데이터 확인
3. 로그 및 메트릭 확인
...
3. 대시보드 이름: `day6-integration-monitoring`
4. 위젯 추가:
   - NAT Gateway 메트릭
   - VPC Peering 메트릭
   - 통합 상태 메트릭
...
**Console 경로**: `NAT Gateway > Integrations`
...
1. 선택: `day6-integration-monitoring`
2. Delete dashboard
3. 확인: "Delete" 클릭
