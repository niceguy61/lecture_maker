# GE Healthcare - 의료 시스템의 구성 관리 및 컴플라이언스

> **Day 24: AWS Config 및 Systems Manager**  
> **주요 AWS 서비스**: Config, Systems Manager, Parameter Store, Patch Manager
...
- **주요 AWS 서비스**: Config, Systems Manager, Parameter Store, Patch Manager
- **사례 출처**: https://aws.amazon.com/architecture/customers/ge-healthcare <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
의료 시스템의 구성 관리 및 컴플라이언스를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > Config
2. **주요 설정**:
   - Region: day24-resource
...
2. **Config** → **Systems Manager** (Day 24의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **Systems Manager** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > Config > Create Resource

**기본 설정**:
- **Name/ID**: `day24-resource`
...
**Console 경로**: Services > Compute > Config

**연결 설정**:
1. Config에서 생성한 리소스 선택
...
```yaml
# day24-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'GE Healthcare - 의료 시스템의 구성 관리 및 컴플라이언스 - Config 구성'

Resources:
  Day24Resource:
    Type: AWS::Config::Resource
    Properties:
      Name: day24-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_config" "day24-resource" {
  name = "day24-resource"
  type = "standard"
  
  tags = {
    Project     = "day24-project"
...
**Console 경로**: CloudWatch > Metrics > Config
...
#### Day 3: EC2 Instances
**연계 방식**: Day 3의 EC2 (Elastic Compute Cloud) 기초와 연계
...
**통합 시나리오 설명**:
- Day 24의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 24 (Config) → Day 23 (CloudTrail)
- Day 23 (CloudTrail) → Day 3 (EC2 Instances)
...
### AWS 공식 문서
- [Config 사용 설명서](https://docs.aws.amazon.com/config/latest/userguide/)
- [Config API 레퍼런스](https://docs.aws.amazon.com/config/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 의료 시스템의 구성 관리 및 컴플라이언스](https://aws.amazon.com/architecture/)
- [Config 베스트 프랙티스](https://docs.aws.amazon.com/config/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [Config 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [GE Healthcare 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/ge-healthcare)
- [AWS re:Invent 발표: GE Healthcare - 의료 시스템의 구성 관리 및 컴플라이언스](https://reinvent.awsevents.com/)
...
**기본 정보**:
- `24`: 일차 번호 (1-28)
- `AWS Config 및 Systems Manager`: 일별 학습 주제 제목
- `GE Healthcare`: 기업명 (실제 또는 가상)
- `의료 시스템의 구성 관리 및 컴플라이언스`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `Config, Systems Manager, Parameter Store, Patch Manager`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `Config`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/config/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/config/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/ge-healthcare`: 기업 블로그 URL
