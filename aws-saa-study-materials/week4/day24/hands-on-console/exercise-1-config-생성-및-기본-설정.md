...
## 🎯 실습 목표
AWS Console을 통해 Config를 생성하고 기본 설정을 구성합니다.
...
**Console 경로**: `Services > AWS Services > Config`

1. AWS Console 상단의 **Services** 메뉴 클릭
2. **AWS Services** 카테고리에서 **Config** 선택
3. 화면 상단의 **"Create config"** 또는 **"Launch config"** 버튼 클릭
...
**상태 확인 방법**:
- Console 경로: `Config > Resources` 또는 `Config > Dashboard`
...
- [ ] Config 리소스가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
