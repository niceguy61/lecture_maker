# Goldman Sachs - 금융 기관의 규정 준수 및 보안 감사

> **Day 23: CloudTrail 및 보안 서비스**  
> **주요 AWS 서비스**: CloudTrail, GuardDuty, Inspector, Macie
...
- **주요 AWS 서비스**: CloudTrail, GuardDuty, Inspector, Macie
- **사례 출처**: https://aws.amazon.com/architecture/customers/goldman-sachs <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
금융 기관의 규정 준수 및 보안 감사를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > CloudTrail
2. **주요 설정**:
   - Region: day23-resource
...
sequenceDiagram
    ...
    participant Service as CloudTrail
...
2. **CloudTrail** → **GuardDuty** (Day 23의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **GuardDuty** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > CloudTrail > Create Resource

**기본 설정**:
- **Name/ID**: `day23-resource`
...
**Console 경로**: Services > Compute > CloudTrail

**연결 설정**:
1. CloudTrail에서 생성한 리소스 선택
...
```yaml
# day23-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Goldman Sachs - 금융 기관의 규정 준수 및 보안 감사 - CloudTrail 구성'

Resources:
  Day23Resource:
    Type: AWS::CloudTrail::Resource
    Properties:
      Name: day23-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_cloudtrail" "day23-resource" {
  name = "day23-resource"
  type = "standard"
  
  tags = {
    Project     = "day23-project"
...
**Console 경로**: CloudWatch > Metrics > CloudTrail
...
**통합 시나리오 설명**:
- Day 23의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 23 (CloudTrail) → Day 2 (IAM Users)
- Day 2 (IAM Users) → Day 5 (VPC)
...
### AWS 공식 문서
- [CloudTrail 사용 설명서](https://docs.aws.amazon.com/cloudtrail/latest/userguide/)
- [CloudTrail API 레퍼런스](https://docs.aws.amazon.com/cloudtrail/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 금융 기관의 규정 준수 및 보안 감사](https://aws.amazon.com/architecture/)
- [CloudTrail 베스트 프랙티스](https://docs.aws.amazon.com/cloudtrail/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [CloudTrail 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Goldman Sachs 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/goldman-sachs)
- [AWS re:Invent 발표: Goldman Sachs - 금융 기관의 규정 준수 및 보안 감사](https://reinvent.awsevents.com/)
...
### 1. CloudTrail의 실제 활용 방법
- CloudTrail의 핵심 기능 이해
- 실제 프로덕션 환경 구성 방법
- 모범 사례 및 안티 패턴
...
**기본 정보**:
- `23`: 일차 번호 (1-28)
- `CloudTrail 및 보안 서비스`: 일별 학습 주제 제목
- `Goldman Sachs`: 기업명 (실제 또는 가상)
- `금융 기관의 규정 준수 및 보안 감사`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `CloudTrail, GuardDuty, Inspector, Macie`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `CloudTrail`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/cloudtrail/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/cloudtrail/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/goldman-sachs`: 기업 블로그 URL
