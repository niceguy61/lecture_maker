...
## 🎯 실습 목표
AWS Console을 통해 CloudTrail를 생성하고 기본 설정을 구성합니다.
...
### Step 1: CloudTrail 생성

**Console 경로**: `Services > Management & Governance > CloudTrail`

1. AWS Console 상단의 **Services** 메뉴 클릭
2. **Management & Governance** 카테고리에서 **CloudTrail** 선택
3. 화면 상단의 **"Create cloudtrail"** 또는 **"Launch cloudtrail"** 버튼 클릭
...
**상태 확인 방법**:
- Console 경로: `CloudTrail > Resources` 또는 `CloudTrail > Dashboard`
...
- [ ] CloudTrail 리소스가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
...
1. 검색: `day23` 또는 `cloudtrail`
2. 선택: 관련 로그 그룹
3. Actions > Delete log group(s)
4. 확인: "Delete" 클릭

### 정리 확인
- [ ] 모든 CloudTrail 리소스가 삭제되었는가?
