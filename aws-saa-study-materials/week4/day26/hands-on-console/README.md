# Hands-On Console: Organizations 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 26에서 학습한 **Organizations**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- Organizations의 핵심 기능을 Console에서 직접 구성
...
- **예상 비용**: $$0.50 - $3.00 (일반적인 실습 비용)
  ...
  - Free Tier 초과: Organizations
- **소요 시간**: 약 60-90분분
- **권장 리전**: ap-northeast-2 (서울)
- **필요한 권한**: OrganizationsFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- Organizations를 중심으로 한 실습 환경
- 주요 구성 요소: Organizations, Control Tower, Service Catalog
...
1. **Organizations 리소스 생성**: AWS Console을 통해 Organizations를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: Organizations 생성 및 기본 설정
**목표**: AWS Console을 통해 Organizations를 생성하고 기본 설정을 구성합니다.
...
1. **Organizations 생성**
   - Console 경로: Services > Networking > Organizations
   - 화면 상단의 "Create organizations" 버튼 클릭
   
2. **기본 설정**
   - **Name**: `day26-organizations`
     - 명명 규칙: day 번호 + 리소스 유형
     - 예시: `day8-my-bucket`
...
4. **태그 추가** (권장)
   ...
   - 다음 태그 추가:
     | Key | Value | 설명 |
     |-----|-------|------|
     | Name | day26-organizations | 리소스 식별 |
...
5. **생성 확인**
   - 모든 설정 검토
   - "Create organizations" 버튼 클릭
   - 생성 완료까지 대기 (약 2-5분)
   - 상태 확인: 리소스 목록에서 상태 컬럼 확인

**검증**:
- [ ] organizations가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
...
### Exercise 2: Control Tower 통합
**목표**: Organizations와 다른 서비스를 통합합니다.
...
1. **Control Tower 생성**
   - Console 경로: Services > Management > Control Tower
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day26-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **Organizations와 연결**
   - "Organizations integration" 또는 "Attach Organizations" 섹션으로 이동
   - 이전에 생성한 `day26-organizations` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day26-organizations-role`
       - 정책: OrganizationsFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: Organizations > organizations > Integrations
     - Control Tower 연결 상태: Connected

**검증**:
- [ ] Control Tower가 정상적으로 생성되었는가?
- [ ] Organizations와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: Organizations > organizations
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day26-Organizations-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day26-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day26-{metric_name}-alert`
     - Alarm description: Organizations 성능 알람
...
2. **기능 테스트**
   - Console 경로: {service} > organizations
   - 기본 기능 테스트:
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day26-Organizations-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/organizations
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: Organizations 생성 완료
- [ ] Exercise 2: Control Tower 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day26-Organizations-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. Control Tower 삭제
- Console 경로: Control Tower > Resources
- 선택: `day26-monitoring`
...
#### 3. Organizations 삭제
- Console 경로: Organizations > Resources
- 선택: `day26-organizations`
- Actions > Delete
- 확인 옵션:
...
- 검색: `day26` 또는 `Organizations`
- 선택: 관련 로그 그룹
- Actions > Delete log group(s)
- 확인: "Delete" 버튼 클릭

### 정리 확인 체크리스트
- [ ] 모든 Organizations 리소스 삭제 완료
- [ ] 모든 Control Tower 리소스 삭제 완료
...
### 핵심 개념
1. **Organizations 아키텍처**: Organizations의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 26 Theory](../theory.md) - Organizations 기본 개념
- [Day 25 Theory](../../day25/theory.md) - 사전 학습 내용
- [Day 27 Theory](../../day27/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Samsung Electronics의 Organizations 활용 사례
- [Best Practices](../best-practices.md) - Organizations 베스트 프랙티스
...
- [Getting Started with Organizations](https://docs.aws.amazon.com/organizations/latest/gsg/)
...
- [ ] Organizations 생성 완료 화면
- [ ] Control Tower 통합 완료 화면
- [ ] CloudWatch 대시보드
- [ ] 테스트 결과 화면
