# Exercise 2: Organizations와 Control Tower 통합

## 🎯 실습 목표
Organizations를 Control Tower와 통합하여 고급 기능을 구성합니다.
...
- [ ] Exercise 1 완료 (day26-organizations 생성됨)
...
graph TB
    ...
    subgraph "AWS Console - Exercise 2"
        Service1[Organizations]
        Service2[Control Tower]
...
**구성 요소**:
- **Organizations**: 메인 리소스
- **Control Tower**: 통합 서비스
...
### Step 1: Control Tower 설정

**Console 경로**: `Services > AWS Services > Control Tower`

1. Control Tower 콘솔로 이동
2. 새 리소스 생성 또는 기존 리소스 선택
3. Organizations와의 통합 옵션 찾기

---

### Step 2: Organizations와 연결

**통합 설정**:
1. Organizations 콘솔로 이동
2. 생성한 리소스 선택 (`day26-organizations`)
3. "Integrations" 또는 "Connections" 탭 선택
4. "Add integration" 또는 "Connect" 버튼 클릭
5. Control Tower 선택 및 설정
...
3. **Use case**: Organizations
4. **Permissions**: 
   - OrganizationsFullAccess
   - Control TowerFullAccess
5. **Role name**: `day26-integration-role`
6. "Create role" 클릭
...
**테스트 방법**:
1. Organizations 콘솔에서 테스트 기능 실행
2. Control Tower에서 데이터 확인
3. 로그 및 메트릭 확인
...
3. 대시보드 이름: `day26-integration-monitoring`
4. 위젯 추가:
   - Organizations 메트릭
   - Control Tower 메트릭
   - 통합 상태 메트릭
...
2. **AWS Console 이벤트 확인**
   - Console 경로: `Organizations > Events`
   - 최근 이벤트 및 상태 변경 확인
   - 실패한 작업의 상세 정보 확인
...
**Console 경로**: `Organizations > Integrations`
...
1. 선택: `day26-integration-monitoring`
2. Delete dashboard
3. 확인: "Delete" 클릭
