# Datadog - 대규모 모니터링 플랫폼의 메트릭 수집 아키텍처
...
- **주요 AWS 서비스**: CloudWatch, Metrics, Logs, Alarms, Dashboards
- **사례 출처**: https://aws.amazon.com/architecture/customers/datadog <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
대규모 모니터링 플랫폼의 메트릭 수집 아키텍처를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > CloudWatch
2. **주요 설정**:
   - Region: day22-resource
...
sequenceDiagram
    ...
    participant Service as CloudWatch
...
2. **CloudWatch** → **Metrics** (Day 22의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **Metrics** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > CloudWatch > Create Resource

**기본 설정**:
- **Name/ID**: `day22-resource`
...
**Console 경로**: Services > Compute > CloudWatch

**연결 설정**:
1. CloudWatch에서 생성한 리소스 선택
...
```yaml
# day22-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Datadog - 대규모 모니터링 플랫폼의 메트릭 수집 아키텍처 - CloudWatch 구성'

Resources:
  Day22Resource:
    Type: AWS::CloudWatch::Resource
    Properties:
      Name: day22-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_cloudwatch" "day22-resource" {
  name = "day22-resource"
  type = "standard"
  
  tags = {
    Project     = "day22-project"
...
**Console 경로**: CloudWatch > Metrics > CloudWatch
...
**통합 시나리오 설명**:
- Day 22의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 22 (CloudWatch) → Day 3 (EC2 Instances)
- Day 3 (EC2 Instances) → Day 10 (RDS)
...
### AWS 공식 문서
- [CloudWatch 사용 설명서](https://docs.aws.amazon.com/cloudwatch/latest/userguide/)
- [CloudWatch API 레퍼런스](https://docs.aws.amazon.com/cloudwatch/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 대규모 모니터링 플랫폼의 메트릭 수집 아키텍처](https://aws.amazon.com/architecture/)
- [CloudWatch 베스트 프랙티스](https://docs.aws.amazon.com/cloudwatch/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [CloudWatch 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Datadog 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/datadog)
- [AWS re:Invent 발표: Datadog - 대규모 모니터링 플랫폼의 메트릭 수집 아키텍처](https://reinvent.awsevents.com/)
...
### 1. CloudWatch의 실제 활용 방법
- CloudWatch의 핵심 기능 이해
- 실제 프로덕션 환경 구성 방법
- 모범 사례 및 안티 패턴
...
**기본 정보**:
- `22`: 일차 번호 (1-28)
- `CloudWatch`: 일별 학습 주제 제목
- `Datadog`: 기업명 (실제 또는 가상)
- `대규모 모니터링 플랫폼의 메트릭 수집 아키텍처`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `CloudWatch, Metrics, Logs, Alarms, Dashboards`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `CloudWatch`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/cloudwatch/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/cloudwatch/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/datadog`: 기업 블로그 URL
