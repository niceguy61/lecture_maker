...
## 🎯 실습 목표
AWS Console을 통해 CloudWatch를 생성하고 기본 설정을 구성합니다.
...
### Step 1: CloudWatch 생성

**Console 경로**: `Services > Management & Governance > CloudWatch`

1. AWS Console 상단의 **Services** 메뉴 클릭
2. **Management & Governance** 카테고리에서 **CloudWatch** 선택
3. 화면 상단의 **"Create cloudwatch"** 또는 **"Launch cloudwatch"** 버튼 클릭
...
**상태 확인 방법**:
- Console 경로: `CloudWatch > Resources` 또는 `CloudWatch > Dashboard`
...
- [ ] CloudWatch 리소스가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
...
1. 검색: `day22` 또는 `cloudwatch`
2. 선택: 관련 로그 그룹
3. Actions > Delete log group(s)
4. 확인: "Delete" 클릭

### 정리 확인
- [ ] 모든 CloudWatch 리소스가 삭제되었는가?
