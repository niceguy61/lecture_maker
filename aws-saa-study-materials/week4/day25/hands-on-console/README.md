# Hands-On Console: KMS 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 25에서 학습한 **KMS**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- KMS의 핵심 기능을 Console에서 직접 구성
...
- **필요한 권한**: KMSFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- KMS를 중심으로 한 실습 환경
- 주요 구성 요소: KMS, CloudHSM, Secrets Manager
...
1. **KMS 리소스 생성**: AWS Console을 통해 KMS를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: KMS 생성 및 기본 설정
**목표**: AWS Console을 통해 KMS를 생성하고 기본 설정을 구성합니다.
...
1. **KMS 생성**
   - Console 경로: Services > Networking > KMS
   - 화면 상단의 "Create kms" 버튼 클릭
...
### Exercise 2: CloudHSM 통합
**목표**: KMS와 다른 서비스를 통합합니다.
...
1. **CloudHSM 생성**
   - Console 경로: Services > Management > CloudHSM
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day25-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **KMS와 연결**
   - "KMS integration" 또는 "Attach KMS" 섹션으로 이동
   - 이전에 생성한 `day25-kms` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day25-kms-role`
       - 정책: KMSFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: KMS > kms > Integrations
     - CloudHSM 연결 상태: Connected

**검증**:
- [ ] CloudHSM가 정상적으로 생성되었는가?
- [ ] KMS와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: KMS > kms
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day25-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day25-{metric_name}-alert`
     - Alarm description: KMS 성능 알람
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day25-KMS-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/kms
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: KMS 생성 완료
- [ ] Exercise 2: CloudHSM 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day25-KMS-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. CloudHSM 삭제
- Console 경로: CloudHSM > Resources
- 선택: `day25-monitoring`
...
### 이론 학습
- [Day 25 Theory](../theory.md) - KMS 기본 개념
- [Day 24 Theory](../../day24/theory.md) - 사전 학습 내용
- [Day 26 Theory](../../day26/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Coinbase의 KMS 활용 사례
- [Best Practices](../best-practices.md) - KMS 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [KMS User Guide](https://docs.aws.amazon.com/kms/latest/userguide/)
...
- [Getting Started with KMS](https://docs.aws.amazon.com/kms/latest/gsg/)
