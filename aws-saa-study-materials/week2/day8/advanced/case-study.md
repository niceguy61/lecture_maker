# Dropbox - 페타바이트급 파일 스토리지 아키텍처

> **Day 8: S3 (Simple Storage Service)**  
> **주요 AWS 서비스**: S3 Buckets, Storage Classes, Lifecycle Policies, Versioning
...
- **주요 AWS 서비스**: S3 Buckets, Storage Classes, Lifecycle Policies, Versioning
- **사례 출처**: https://aws.amazon.com/architecture/customers/dropbox <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
페타바이트급 파일 스토리지 아키텍처를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > S3 Buckets
2. **주요 설정**:
   - Region: day8-resource
...
**역할**: 비용 최적화를 위한 스토리지 클래스 선택

**구성 방법**:
- Standard, IA, Glacier 등 스토리지 클래스 선택
...
sequenceDiagram
    ...
    participant Service as S3 Buckets
...
2. **S3 Buckets** → **Storage Classes** (Day 8의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **Storage Classes** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > S3 Buckets > Create Resource

**기본 설정**:
- **Name/ID**: `day8-resource`
...
**Console 경로**: Services > Compute > S3 Buckets

**연결 설정**:
1. S3 Buckets에서 생성한 리소스 선택
...
```yaml
# day8-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Dropbox - 페타바이트급 파일 스토리지 아키텍처 - S3 Buckets 구성'

Resources:
  Day8Resource:
    Type: AWS::S3Buckets::Resource
    Properties:
      Name: day8-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_s3_buckets" "day8-resource" {
  name = "day8-resource"
  type = "standard"
  
  tags = {
    Project     = "day8-project"
...
**Console 경로**: CloudWatch > Metrics > S3Buckets
...
**통합 시나리오 설명**:
- Day 8의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 8 (S3 Buckets) → Day 1 (Regions)
...
### AWS 공식 문서
- [S3 Buckets 사용 설명서](https://docs.aws.amazon.com/s3-buckets/latest/userguide/)
- [S3 Buckets API 레퍼런스](https://docs.aws.amazon.com/s3-buckets/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 페타바이트급 파일 스토리지 아키텍처](https://aws.amazon.com/architecture/)
- [S3 Buckets 베스트 프랙티스](https://docs.aws.amazon.com/s3-buckets/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [S3 Buckets 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Dropbox 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/dropbox)
- [AWS re:Invent 발표: Dropbox - 페타바이트급 파일 스토리지 아키텍처](https://reinvent.awsevents.com/)
...
### 1. S3 Buckets의 실제 활용 방법
- S3 Buckets의 핵심 기능 이해
- 실제 프로덕션 환경 구성 방법
- 모범 사례 및 안티 패턴
...
**기본 정보**:
- `8`: 일차 번호 (1-28)
- `S3 (Simple Storage Service)`: 일별 학습 주제 제목
- `Dropbox`: 기업명 (실제 또는 가상)
- `페타바이트급 파일 스토리지 아키텍처`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `S3 Buckets, Storage Classes, Lifecycle Policies, Versioning`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `S3 Buckets`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/s3-buckets/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/s3-buckets/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/dropbox`: 기업 블로그 URL
