# Hands-On Console: S3 Buckets 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 8에서 학습한 **S3 Buckets**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- S3 Buckets의 핵심 기능을 Console에서 직접 구성
...
- **예상 비용**: $$0.10 - $0.50 (소량 데이터 저장 및 전송)
  ...
  - Free Tier 초과: S3 Buckets
- **소요 시간**: 약 30-45분분
- **권장 리전**: ap-northeast-2 (서울)
- **필요한 권한**: S3 BucketsFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- S3 Buckets를 중심으로 한 실습 환경
- 주요 구성 요소: S3 Buckets, Storage Classes, Lifecycle Policies
...
1. **S3 Buckets 리소스 생성**: AWS Console을 통해 S3 Buckets를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: S3 Buckets 생성 및 기본 설정
**목표**: AWS Console을 통해 S3 Buckets를 생성하고 기본 설정을 구성합니다.
...
1. **S3 Buckets 생성**
   - Console 경로: Services > Storage > S3 Buckets
   - 화면 상단의 "Create s3" 버튼 클릭
   
2. **기본 설정**
   - **Name**: `day8-s3-buckets`
     - 명명 규칙: day 번호 + 리소스 유형
     - 예시: `day8-my-bucket`
...
### Exercise 2: Storage Classes 통합
**목표**: S3 Buckets와 다른 서비스를 통합합니다.

**예상 소요 시간**: 10-15분

**단계**:

1. **Storage Classes 생성**
   - Console 경로: Services > Management > Storage Classes
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day8-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **S3 Buckets와 연결**
   - "S3 Buckets integration" 또는 "Attach S3 Buckets" 섹션으로 이동
   - 이전에 생성한 `day8-s3-buckets` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day8-s3-buckets-role`
       - 정책: S3 BucketsFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: S3 Buckets > s3-buckets > Integrations
     - Storage Classes 연결 상태: Connected

**검증**:
- [ ] Storage Classes가 정상적으로 생성되었는가?
- [ ] S3 Buckets와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: S3 Buckets > s3-buckets
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day8-S3 Buckets-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day8-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day8-{metric_name}-alert`
     - Alarm description: S3 Buckets 성능 알람
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day8-S3 Buckets-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/s3/buckets
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: S3 Buckets 생성 완료
- [ ] Exercise 2: Storage Classes 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day8-S3 Buckets-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. Storage Classes 삭제
- Console 경로: Storage Classes > Resources
- 선택: `day8-monitoring`
...
#### 3. S3 Buckets 삭제
- Console 경로: S3 Buckets > Resources
- 선택: `day8-s3-buckets`
- Actions > Delete
- 확인 옵션:
...
- 검색: `day8` 또는 `S3 Buckets`
- 선택: 관련 로그 그룹
- Actions > Delete log group(s)
- 확인: "Delete" 버튼 클릭

### 정리 확인 체크리스트
- [ ] 모든 S3 Buckets 리소스 삭제 완료
- [ ] 모든 Storage Classes 리소스 삭제 완료
...
### 핵심 개념
1. **S3 Buckets 아키텍처**: S3 Buckets의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 8 Theory](../theory.md) - S3 Buckets 기본 개념
- [Day 7 Theory](../../day7/theory.md) - 사전 학습 내용
- [Day 9 Theory](../../day9/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Dropbox의 S3 Buckets 활용 사례
- [Best Practices](../best-practices.md) - S3 Buckets 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [S3 Buckets User Guide](https://docs.aws.amazon.com/s3-buckets/latest/userguide/)
- [S3 Buckets API Reference](https://docs.aws.amazon.com/s3-buckets/latest/APIReference/)
- [AWS Console User Guide](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/)
- [Getting Started with S3 Buckets](https://docs.aws.amazon.com/s3-buckets/latest/gsg/)
