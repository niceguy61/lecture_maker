...
## 🎯 실습 목표
AWS Console을 통해 S3 Buckets를 생성하고 기본 설정을 구성합니다.
...
### Step 1: S3 Buckets 생성

**Console 경로**: `Services > Storage > S3 Buckets`

1. AWS Console 상단의 **Services** 메뉴 클릭
2. **Storage** 카테고리에서 **S3 Buckets** 선택
3. 화면 상단의 **"Create s3"** 또는 **"Launch s3"** 버튼 클릭
...
**기본 구성**:
- **Bucket Name**: `day{day_number}-{your-name}-bucket` (전역 고유 이름)
...
**상태 확인 방법**:
- Console 경로: `S3 Buckets > Resources` 또는 `S3 Buckets > Dashboard`
...
- [ ] S3 Buckets 리소스가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
