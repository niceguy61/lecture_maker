...
## 🎯 실습 목표
AWS Console을 통해 DynamoDB를 생성하고 기본 설정을 구성합니다.
...
2. **Database** 카테고리에서 **DynamoDB** 선택
3. 화면 상단의 **"Create dynamodb"** 또는 **"Launch dynamodb"** 버튼 클릭
...
**기본 구성**:
- **Table Name**: `day{day_number}-table`
...
**상태 확인 방법**:
- Console 경로: `DynamoDB > Resources` 또는 `DynamoDB > Dashboard`
...
- [ ] DynamoDB 리소스가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
...
1. 검색: `day11` 또는 `dynamodb`
2. 선택: 관련 로그 그룹
3. Actions > Delete log group(s)
4. 확인: "Delete" 클릭

### 정리 확인
- [ ] 모든 DynamoDB 리소스가 삭제되었는가?
