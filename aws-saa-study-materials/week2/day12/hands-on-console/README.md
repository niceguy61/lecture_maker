# Hands-On Console: ElastiCache 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 12에서 학습한 **ElastiCache**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- ElastiCache의 핵심 기능을 Console에서 직접 구성
...
### 실습 환경 정보
- **예상 비용**: $$1.00 - $5.00 (최소 인스턴스 사용 시)
  - Free Tier 범위 내: 해당 없음
  - Free Tier 초과: ElastiCache
- **소요 시간**: 약 60-90분분
- **권장 리전**: ap-northeast-2 (서울)
- **필요한 권한**: ElastiCacheFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- ElastiCache를 중심으로 한 실습 환경
- 주요 구성 요소: ElastiCache, Redis, Memcached
...
1. **ElastiCache 리소스 생성**: AWS Console을 통해 ElastiCache를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: ElastiCache 생성 및 기본 설정
**목표**: AWS Console을 통해 ElastiCache를 생성하고 기본 설정을 구성합니다.
...
1. **ElastiCache 생성**
   - Console 경로: Services > Networking > ElastiCache
   - 화면 상단의 "Create elasticache" 버튼 클릭
   
2. **기본 설정**
   - **Name**: `day12-elasticache`
     - 명명 규칙: day 번호 + 리소스 유형
     - 예시: `day8-my-bucket`
...
5. **생성 확인**
   - 모든 설정 검토
   - "Create elasticache" 버튼 클릭
   - 생성 완료까지 대기 (약 2-5분)
   - 상태 확인: 리소스 목록에서 상태 컬럼 확인

**검증**:
- [ ] elasticache가 "Available" 또는 "Active" 상태인가?
- [ ] 리소스 이름이 올바르게 설정되었는가?
...
1. **Redis 생성**
   - Console 경로: Services > Management > Redis
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day12-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **ElastiCache와 연결**
   - "ElastiCache integration" 또는 "Attach ElastiCache" 섹션으로 이동
   - 이전에 생성한 `day12-elasticache` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day12-elasticache-role`
       - 정책: ElastiCacheFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: ElastiCache > elasticache > Integrations
     - Redis 연결 상태: Connected

**검증**:
- [ ] Redis가 정상적으로 생성되었는가?
- [ ] ElastiCache와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: ElastiCache > elasticache
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day12-ElastiCache-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day12-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day12-{metric_name}-alert`
     - Alarm description: ElastiCache 성능 알람
...
2. **기능 테스트**
   - Console 경로: {service} > elasticache
   - 기본 기능 테스트:
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day12-ElastiCache-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/elasticache
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: ElastiCache 생성 완료
- [ ] Exercise 2: Redis 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day12-ElastiCache-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. Redis 삭제
- Console 경로: Redis > Resources
- 선택: `day12-monitoring`
...
#### 3. ElastiCache 삭제
- Console 경로: ElastiCache > Resources
- 선택: `day12-elasticache`
- Actions > Delete
- 확인 옵션:
...
- 검색: `day12` 또는 `ElastiCache`
- 선택: 관련 로그 그룹
- Actions > Delete log group(s)
- 확인: "Delete" 버튼 클릭

### 정리 확인 체크리스트
- [ ] 모든 ElastiCache 리소스 삭제 완료
- [ ] 모든 Redis 리소스 삭제 완료
...
- 확인 항목:
  - 이번 달 누적 비용: $$1.00 - $5.00
  - ElastiCache 관련 비용: $$1.00 - $5.00
  - 예상 월말 비용: $$1.00 - $5.00

**예상 비용**:
- Free Tier 범위 내 실습: $0.00
- Free Tier 초과 시: $$1.00 - $5.00
...
### 핵심 개념
1. **ElastiCache 아키텍처**: ElastiCache의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 12 Theory](../theory.md) - ElastiCache 기본 개념
- [Day 11 Theory](../../day11/theory.md) - 사전 학습 내용
- [Day 13 Theory](../../day13/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Tinder의 ElastiCache 활용 사례
- [Best Practices](../best-practices.md) - ElastiCache 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [ElastiCache User Guide](https://docs.aws.amazon.com/elasticache/latest/userguide/)
- [ElastiCache API Reference](https://docs.aws.amazon.com/elasticache/latest/APIReference/)
- [AWS Console User Guide](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/)
- [Getting Started with ElastiCache](https://docs.aws.amazon.com/elasticache/latest/gsg/)
