# Hands-On Console: ALB 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 13에서 학습한 **ALB**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- ALB의 핵심 기능을 Console에서 직접 구성
...
- **필요한 권한**: ALBFullAccess 또는 관리자 권한
...
1. **ALB 리소스 생성**: AWS Console을 통해 ALB를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: ALB 생성 및 기본 설정
**목표**: AWS Console을 통해 ALB를 생성하고 기본 설정을 구성합니다.
...
1. **ALB 생성**
   - Console 경로: Services > Networking > ALB
   - 화면 상단의 "Create alb" 버튼 클릭
...
1. **NLB 생성**
   - Console 경로: Services > Management > NLB
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day13-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **ALB와 연결**
   - "ALB integration" 또는 "Attach ALB" 섹션으로 이동
   - 이전에 생성한 `day13-alb` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day13-alb-role`
       - 정책: ALBFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: ALB > alb > Integrations
     - NLB 연결 상태: Connected
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day13-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day13-{metric_name}-alert`
     - Alarm description: ALB 성능 알람
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day13-ALB-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/alb
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: ALB 생성 완료
- [ ] Exercise 2: NLB 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day13-ALB-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. NLB 삭제
- Console 경로: NLB > Resources
- 선택: `day13-monitoring`
...
### 이론 학습
- [Day 13 Theory](../theory.md) - ALB 기본 개념
- [Day 12 Theory](../../day12/theory.md) - 사전 학습 내용
- [Day 14 Theory](../../day14/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Pinterest의 ALB 활용 사례
- [Best Practices](../best-practices.md) - ALB 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [ALB User Guide](https://docs.aws.amazon.com/alb/latest/userguide/)
- [ALB API Reference](https://docs.aws.amazon.com/alb/latest/APIReference/)
- [AWS Console User Guide](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/)
- [Getting Started with ALB](https://docs.aws.amazon.com/alb/latest/gsg/)
