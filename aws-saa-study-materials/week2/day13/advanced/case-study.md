# Pinterest - 이미지 공유 플랫폼의 트래픽 분산 아키텍처
...
- **주요 AWS 서비스**: ALB, NLB, CLB, Target Groups
- **사례 출처**: https://aws.amazon.com/architecture/customers/pinterest <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
이미지 공유 플랫폼의 트래픽 분산 아키텍처를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > ALB
2. **주요 설정**:
   - Region: day13-resource
...
**역할**: 네트워크 계층 로드 밸런싱 (TCP/UDP)

**구성 방법**:
- TCP/UDP 리스너 및 대상 그룹 구성
...
**Console 경로**: Services > Compute > ALB > Create Resource

**기본 설정**:
- **Name/ID**: `day13-resource`
...
**Console 경로**: Services > Compute > ALB
...
```yaml
# day13-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Pinterest - 이미지 공유 플랫폼의 트래픽 분산 아키텍처 - ALB 구성'

Resources:
  Day13Resource:
    Type: AWS::ALB::Resource
    Properties:
      Name: day13-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_alb" "day13-resource" {
  name = "day13-resource"
  type = "standard"
  
  tags = {
    Project     = "day13-project"
...
**Console 경로**: CloudWatch > Metrics > ALB
...
**통합 시나리오 설명**:
- Day 13의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 13 (ALB) → Day 3 (EC2 Instances)
...
### AWS 공식 문서
- [ALB 사용 설명서](https://docs.aws.amazon.com/alb/latest/userguide/)
- [ALB API 레퍼런스](https://docs.aws.amazon.com/alb/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 이미지 공유 플랫폼의 트래픽 분산 아키텍처](https://aws.amazon.com/architecture/)
- [ALB 베스트 프랙티스](https://docs.aws.amazon.com/alb/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [ALB 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Pinterest 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/pinterest)
- [AWS re:Invent 발표: Pinterest - 이미지 공유 플랫폼의 트래픽 분산 아키텍처](https://reinvent.awsevents.com/)
...
**기본 정보**:
- `13`: 일차 번호 (1-28)
- `ELB (Elastic Load Balancing)`: 일별 학습 주제 제목
- `Pinterest`: 기업명 (실제 또는 가상)
- `이미지 공유 플랫폼의 트래픽 분산 아키텍처`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `ALB, NLB, CLB, Target Groups`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `ALB`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/alb/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/alb/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/pinterest`: 기업 블로그 URL
