# Expedia - 글로벌 여행 예약 시스템의 관계형 데이터베이스

> **Day 10: RDS (Relational Database Service)**  
> **주요 AWS 서비스**: RDS, Multi-AZ, Read Replicas, Aurora
...
- **주요 AWS 서비스**: RDS, Multi-AZ, Read Replicas, Aurora
- **사례 출처**: https://aws.amazon.com/architecture/customers/expedia <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
글로벌 여행 예약 시스템의 관계형 데이터베이스를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > RDS
2. **주요 설정**:
   - Region: day10-resource
...
2. **RDS** → **Multi-AZ** (Day 10의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **Multi-AZ** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > RDS > Create Resource

**기본 설정**:
- **Name/ID**: `day10-resource`
...
**Console 경로**: Services > Compute > RDS
...
```yaml
# day10-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Expedia - 글로벌 여행 예약 시스템의 관계형 데이터베이스 - RDS 구성'

Resources:
  Day10Resource:
    Type: AWS::RDS::Resource
    Properties:
      Name: day10-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_rds" "day10-resource" {
  name = "day10-resource"
  type = "standard"
  
  tags = {
    Project     = "day10-project"
...
**통합 시나리오 설명**:
- Day 10의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 10 (RDS) → Day 11 (DynamoDB)
- Day 11 (DynamoDB) → Day 5 (VPC)
...
- [RDS API 레퍼런스](https://docs.aws.amazon.com/rds/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 글로벌 여행 예약 시스템의 관계형 데이터베이스](https://aws.amazon.com/architecture/)
- [RDS 베스트 프랙티스](https://docs.aws.amazon.com/rds/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [RDS 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Expedia 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/expedia)
- [AWS re:Invent 발표: Expedia - 글로벌 여행 예약 시스템의 관계형 데이터베이스](https://reinvent.awsevents.com/)
...
**기본 정보**:
- `10`: 일차 번호 (1-28)
- `RDS (Relational Database Service)`: 일별 학습 주제 제목
- `Expedia`: 기업명 (실제 또는 가상)
- `글로벌 여행 예약 시스템의 관계형 데이터베이스`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `RDS, Multi-AZ, Read Replicas, Aurora`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `RDS`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/rds/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/rds/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/expedia`: 기업 블로그 URL
