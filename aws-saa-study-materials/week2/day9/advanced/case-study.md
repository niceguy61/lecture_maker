# Adobe - 크리에이티브 클라우드를 위한 고성능 스토리지
...
- **주요 AWS 서비스**: EBS Volumes, EBS Snapshots, EFS, FSx
- **사례 출처**: https://aws.amazon.com/architecture/customers/adobe <!-- 공개 자료 링크 또는 "AWS Well-Architected Framework 기반" -->
...
크리에이티브 클라우드를 위한 고성능 스토리지를 위한 확장 가능하고 안정적인 인프라 구축
...
1. **Console 경로**: Services > Storage > EBS Volumes
2. **주요 설정**:
   - Region: day9-resource
...
sequenceDiagram
    ...
    participant Service as EBS Volumes
...
**플로우 설명**:
1. **사용자 요청** → EBS Volumes
   - 사용자 요청을 받아 처리 시작
   
2. **EBS Volumes** → **EBS Snapshots** (Day 9의 주요 서비스)
   - 비즈니스 로직 처리 및 데이터 변환
   
3. **EBS Snapshots** → **Data Storage**
   - 데이터 저장 및 영속화
...
**Console 경로**: Services > Compute > EBS Volumes > Create Resource

**기본 설정**:
- **Name/ID**: `day9-resource`
...
**Console 경로**: Services > Compute > EBS Volumes

**연결 설정**:
1. EBS Volumes에서 생성한 리소스 선택
...
```yaml
# day9-resource-stack.yaml
AWSTemplateFormatVersion: '2010-09-09'
Description: 'Adobe - 크리에이티브 클라우드를 위한 고성능 스토리지 - EBS Volumes 구성'

Resources:
  Day9Resource:
    Type: AWS::EBSVolumes::Resource
    Properties:
      Name: day9-resource
      Type: Standard
      Tags:
...
```hcl
# main.tf
resource "aws_ebs_volumes" "day9-resource" {
  name = "day9-resource"
  type = "standard"
  
  tags = {
    Project     = "day9-project"
...
**Console 경로**: CloudWatch > Metrics > EBSVolumes
...
**통합 시나리오 설명**:
- Day 9의 서비스들이 전체 아키텍처에서 핵심 역할 수행

**서비스 의존성**:
- Day 9 (EBS Volumes) → Day 3 (EC2 Instances)
- Day 3 (EC2 Instances) → Day 8 (S3 Buckets)
...
### AWS 공식 문서
- [EBS Volumes 사용 설명서](https://docs.aws.amazon.com/ebs-volumes/latest/userguide/)
- [EBS Volumes API 레퍼런스](https://docs.aws.amazon.com/ebs-volumes/latest/userguide/)
- [AWS Well-Architected Framework - Operational Excellence](https://docs.aws.amazon.com/wellarchitected/latest/framework/)

### 아키텍처 및 베스트 프랙티스
- [AWS 아키텍처 센터 - 크리에이티브 클라우드를 위한 고성능 스토리지](https://aws.amazon.com/architecture/)
- [EBS Volumes 베스트 프랙티스](https://docs.aws.amazon.com/ebs-volumes/latest/userguide/)
- [보안 베스트 프랙티스 - IAM Best Practices](https://docs.aws.amazon.com/security/)

### 비용 최적화
- [AWS 요금 계산기](https://calculator.aws/)
- [EBS Volumes 요금 안내](https://aws.amazon.com/pricing)
- [비용 최적화 가이드](https://docs.aws.amazon.com/cost-management/)

### 기업 사례 및 발표 자료
- [Adobe 공식 블로그 포스트](https://aws.amazon.com/architecture/customers/adobe)
- [AWS re:Invent 발표: Adobe - 크리에이티브 클라우드를 위한 고성능 스토리지](https://reinvent.awsevents.com/)
...
### 1. EBS Volumes의 실제 활용 방법
- EBS Volumes의 핵심 기능 이해
- 실제 프로덕션 환경 구성 방법
- 모범 사례 및 안티 패턴
...
**기본 정보**:
- `9`: 일차 번호 (1-28)
- `EBS 및 EFS`: 일별 학습 주제 제목
- `Adobe`: 기업명 (실제 또는 가상)
- `크리에이티브 클라우드를 위한 고성능 스토리지`: 사례 연구 초점 (예: "글로벌 스트리밍 아키텍처")

**서비스 정보**:
- `EBS Volumes, EBS Snapshots, EFS, FSx`: 주요 AWS 서비스 목록 (쉼표로 구분)
- `EBS Volumes`: 주요 서비스 단수형
- `CloudWatch`: 연계 서비스명
...
**URL 및 링크**:
- `https://docs.aws.amazon.com/ebs-volumes/latest/userguide/`: AWS 공식 문서 URL
- `https://docs.aws.amazon.com/ebs-volumes/latest/userguide/`: API 레퍼런스 URL
- `https://aws.amazon.com/architecture/customers/adobe`: 기업 블로그 URL
