# Hands-On Console: EBS Volumes 실습 가이드

## 🎯 실습 개요
이 실습에서는 Day 9에서 학습한 **EBS Volumes**을 AWS Console을 통해 직접 구성하고 테스트합니다.

**학습 목표**:
- EBS Volumes의 핵심 기능을 Console에서 직접 구성
...
### 실습 환경 정보
- **예상 비용**: $$0.10 - $0.50 (소량 데이터 저장 및 전송)
  - Free Tier 범위 내: 해당 없음
  - Free Tier 초과: EBS Volumes
- **소요 시간**: 약 60-90분분
- **권장 리전**: ap-northeast-2 (서울)
- **필요한 권한**: EBS VolumesFullAccess 또는 관리자 권한
...
**아키텍처 설명**:
- EBS Volumes를 중심으로 한 실습 환경
- 주요 구성 요소: EBS Volumes, EBS Snapshots, EFS
...
1. **EBS Volumes 리소스 생성**: AWS Console을 통해 EBS Volumes를 생성하고 기본 설정을 구성합니다
...
### Exercise 1: EBS Volumes 생성 및 기본 설정
**목표**: AWS Console을 통해 EBS Volumes를 생성하고 기본 설정을 구성합니다.
...
1. **EBS Volumes 생성**
   - Console 경로: Services > Networking > EBS Volumes
   - 화면 상단의 "Create ebs" 버튼 클릭
   
2. **기본 설정**
   - **Name**: `day9-ebs-volumes`
     - 명명 규칙: day 번호 + 리소스 유형
     - 예시: `day8-my-bucket`
...
### Exercise 2: EBS Snapshots 통합
**목표**: EBS Volumes와 다른 서비스를 통합합니다.
...
1. **EBS Snapshots 생성**
   - Console 경로: Services > Management > EBS Snapshots
   - "Create monitoring" 버튼 클릭

2. **기본 설정**
   - **Name**: `day9-monitoring`
   - **Region**: `ap-northeast-2` (서울)
   - **Interval**: 5 minutes
   - **Retention**: 7 days

3. **EBS Volumes와 연결**
   - "EBS Volumes integration" 또는 "Attach EBS Volumes" 섹션으로 이동
   - 이전에 생성한 `day9-ebs-volumes` 선택
   - 연결 옵션 구성:
     - **Auto-enable**: Yes
...
4. **권한 설정**
   ...
   - 옵션 선택:
     - [ ] 기존 역할 사용: 기존 역할 없음
     - [x] 새 역할 생성 (권장)
       - 역할 이름: `day9-ebs-volumes-role`
       - 정책: EBS VolumesFullAccess, CloudWatchFullAccess

5. **생성 및 연결 확인**
   - "Create" 또는 "Attach" 버튼 클릭
   - 생성 완료 대기
   - 연결 상태 확인:
     - Console 경로: EBS Volumes > ebs-volumes > Integrations
     - EBS Snapshots 연결 상태: Connected

**검증**:
- [ ] EBS Snapshots가 정상적으로 생성되었는가?
- [ ] EBS Volumes와 연결이 완료되었는가?
- [ ] IAM 역할이 올바르게 설정되었는가?
- [ ] 연결 테스트가 성공하는가?

**연결 테스트 방법**:
1. Console 경로: EBS Volumes > ebs-volumes
...
1. **CloudWatch 대시보드 생성**
   ...
   - 대시보드 이름: `day9-EBS Volumes-monitoring`
...
4. **알람 생성**
   ...
   - **알림 설정**:
     ...
     - Topic name: `day9-alerts`
     - Email endpoint: your-email@example.com
     - "Create topic" 클릭
   
   - **알람 이름**:
     - Alarm name: `day9-{metric_name}-alert`
     - Alarm description: EBS Volumes 성능 알람
...
2. **기능 테스트**
   - Console 경로: {service} > ebs-volumes
   - 기본 기능 테스트:
...
3. **성능 확인**
   - Console 경로: CloudWatch > Dashboards > day9-EBS Volumes-monitoring
   - 확인 항목:
     - 응답 시간: < 100ms
     - 처리량: > 100 TPS
     - 에러율: < 1%
...
4. **로그 확인**
   - Console 경로: CloudWatch > Log groups > /aws/ebs/volumes
   - 최근 로그 스트림 선택
   - 확인 항목:
     - 에러 메시지 없음
     - 성공 로그 확인
...
### 생성된 리소스 확인
- [ ] Exercise 1: EBS Volumes 생성 완료
- [ ] Exercise 2: EBS Snapshots 통합 완료
...
**대시보드 삭제**:
- Console 경로: CloudWatch > Dashboards
- 선택: `day9-EBS Volumes-monitoring`
- Delete dashboard
- 확인: "Delete" 버튼 클릭

#### 2. EBS Snapshots 삭제
- Console 경로: EBS Snapshots > Resources
- 선택: `day9-monitoring`
...
#### 3. EBS Volumes 삭제
- Console 경로: EBS Volumes > Resources
- 선택: `day9-ebs-volumes`
- Actions > Delete
- 확인 옵션:
...
- 검색: `day9` 또는 `EBS Volumes`
- 선택: 관련 로그 그룹
- Actions > Delete log group(s)
- 확인: "Delete" 버튼 클릭

### 정리 확인 체크리스트
- [ ] 모든 EBS Volumes 리소스 삭제 완료
- [ ] 모든 EBS Snapshots 리소스 삭제 완료
...
- 확인 항목:
  - 이번 달 누적 비용: $$0.10 - $0.50
  - EBS Volumes 관련 비용: $$0.10 - $0.50
  - 예상 월말 비용: $$0.10 - $0.50

**예상 비용**:
- Free Tier 범위 내 실습: $0.00
- Free Tier 초과 시: $$0.10 - $0.50
...
### 핵심 개념
1. **EBS Volumes 아키텍처**: EBS Volumes의 기본 구조 및 동작 원리
...
### 이론 학습
- [Day 9 Theory](../theory.md) - EBS Volumes 기본 개념
- [Day 8 Theory](../../day8/theory.md) - 사전 학습 내용
- [Day 10 Theory](../../day10/theory.md) - 다음 학습 내용

### 심화 학습
- [Case Study](../case-study.md) - Adobe의 EBS Volumes 활용 사례
- [Best Practices](../best-practices.md) - EBS Volumes 베스트 프랙티스
- [Troubleshooting](../troubleshooting.md) - 일반적인 문제 및 해결 방법

### AWS 공식 문서
- [EBS Volumes User Guide](https://docs.aws.amazon.com/ebs-volumes/latest/userguide/)
- [EBS Volumes API Reference](https://docs.aws.amazon.com/ebs-volumes/latest/APIReference/)
- [AWS Console User Guide](https://docs.aws.amazon.com/awsconsolehelpdocs/latest/gsg/)
- [Getting Started with EBS Volumes](https://docs.aws.amazon.com/ebs-volumes/latest/gsg/)
...
- [ ] EBS Volumes 생성 완료 화면
- [ ] EBS Snapshots 통합 완료 화면
- [ ] CloudWatch 대시보드
- [ ] 테스트 결과 화면
