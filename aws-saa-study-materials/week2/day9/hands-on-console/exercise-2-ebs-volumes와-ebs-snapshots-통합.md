# Exercise 2: EBS Volumes와 EBS Snapshots 통합

## 🎯 실습 목표
EBS Volumes를 EBS Snapshots와 통합하여 고급 기능을 구성합니다.
...
- [ ] Exercise 1 완료 (day9-ebs-volumes 생성됨)
...
graph TB
    ...
    subgraph "AWS Console - Exercise 2"
        Service1[EBS Volumes]
        Service2[EBS Snapshots]
...
### Step 1: EBS Snapshots 설정

**Console 경로**: `Services > Storage > EBS Snapshots`

1. EBS Snapshots 콘솔로 이동
2. 새 리소스 생성 또는 기존 리소스 선택
3. EBS Volumes와의 통합 옵션 찾기

---

### Step 2: EBS Volumes와 연결

**통합 설정**:
1. EBS Volumes 콘솔로 이동
2. 생성한 리소스 선택 (`day9-ebs-volumes`)
3. "Integrations" 또는 "Connections" 탭 선택
4. "Add integration" 또는 "Connect" 버튼 클릭
5. EBS Snapshots 선택 및 설정
...
3. **Use case**: EBS Volumes
4. **Permissions**: 
   - EBS VolumesFullAccess
   - EBS SnapshotsFullAccess
5. **Role name**: `day9-integration-role`
6. "Create role" 클릭
...
**테스트 방법**:
1. EBS Volumes 콘솔에서 테스트 기능 실행
2. EBS Snapshots에서 데이터 확인
3. 로그 및 메트릭 확인
...
3. 대시보드 이름: `day9-integration-monitoring`
4. 위젯 추가:
   - EBS Volumes 메트릭
   - EBS Snapshots 메트릭
   - 통합 상태 메트릭
...
**Console 경로**: `EBS Volumes > Integrations`
...
1. 선택: `day9-integration-monitoring`
2. Delete dashboard
3. 확인: "Delete" 클릭
